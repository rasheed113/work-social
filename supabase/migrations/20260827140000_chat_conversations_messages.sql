...
create table if not exists public.conversations (
  ...
  title text, avatar_url text, created_by uuid not null references public.profiles(id) on delete cascade,
  direct_user_a uuid references public.profiles(id) on delete cascade, direct_user_b uuid references public.profiles(id) on delete cascade,
  direct_key text, created_at timestamptz not null default now(), updated_at timestamptz not null default now(),
...
begin update public.conversations set updated_at = new.created_at where id = new.conversation_id; return new; end;
$$;
...
create policy conversation_members_delete_authorized on public.conversation_members for delete to authenticated using (profile_id = auth.uid() or exists (select 1 from public.conversations c where c.id = conversation_id and c.created_by = auth.uid()));
