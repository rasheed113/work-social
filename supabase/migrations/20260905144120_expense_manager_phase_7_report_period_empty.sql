...
as $$
with params as (
  select period_start as start_date, period_end as end_date,
         (period_start - interval '1 month')::date as previous_start,
         (period_start - interval '1 day')::date as previous_end,
         (period_start - interval '5 months')::date as trend_start
),
period_summary as (
  select a.currency,
         sum(case when t.type = 'income' then t.amount else 0 end)::numeric as income,
         sum(case when t.type = 'expense' then t.amount else 0 end)::numeric as expenses,
         count(t.id)::bigint as transaction_count
  from public.expense_accounts a
  join public.expense_transactions t on t.user_id = a.user_id and t.account_id = a.id
...
trend_currencies as (
  ...
  from public.expense_transactions t
  join public.expense_accounts a on a.id = t.account_id and a.user_id = t.user_id
  where t.user_id = (select auth.uid()) and t.type in ('income','expense')
    and t.date >= (select trend_start from params) and t.date <= (select end_date from params)
),
monthly_trend as (
  select m.month_start::date as month_start, a.currency,
         ...
         count(t.id)::bigint as transaction_count
  from generate_series((select trend_start from params), (select start_date from params), interval '1 month') m(month_start)
  cross join trend_currencies a
  left join public.expense_accounts acc on acc.user_id = (select auth.uid()) and acc.currency = a.currency
  left join public.expense_transactions t on t.user_id = (select auth.uid()) and t.account_id = acc.id
...
select jsonb_build_object(
  ...
  'has_any_transactions', exists(select 1 from public.expense_transactions where user_id = (select auth.uid())),
