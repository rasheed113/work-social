...
as $$
  ...
  where name in ('worker_diary_vapid_private_key','worker_diary_vapid_subject','worker_diary_cron_secret','worker_diary_project_url');
$$;
...
begin
  ...
  with due as (
    select r.id
    from public.worker_diary_reminders r
    ...
    where r.enabled = true
      ...
      and ((r.reminder_kind = 'todo' and p.todo_reminders_enabled = true and e.completed = false)
        or (r.reminder_kind = 'event' and p.event_reminders_enabled = true))
...
create index if not exists worker_diary_reminders_processing_idx
  on public.worker_diary_reminders (claimed_at, status)
