create table if not exists public.contractor_team_work_entry_reports (
...
create unique index if not exists contractor_team_work_entry_reports_one_pending
  on public.contractor_team_work_entry_reports(work_entry_id)
  where status = 'pending';

create index if not exists contractor_team_work_entry_reports_worker_status_idx
  on public.contractor_team_work_entry_reports(worker_profile_id,status,created_at desc);

create index if not exists contractor_team_work_entry_reports_contractor_idx
  on public.contractor_team_work_entry_reports(contractor_profile_id,created_at desc);

alter table public.contractor_team_work_entry_reports enable row level security;
revoke all on public.contractor_team_work_entry_reports from anon, authenticated;
grant select on public.contractor_team_work_entry_reports to authenticated;
...
begin
  ...
  if p_entry_id is null or char_length(btrim(coalesce(p_note,''))) = 0 then
    raise exception 'A report note is required';
  end if;

  select e.team_id,e.worker_profile_id,t.team_number
    into v_team_id,v_worker,v_team_number
