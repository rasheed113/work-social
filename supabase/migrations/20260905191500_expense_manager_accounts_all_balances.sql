...
with account_balances as (
  select a.id, a.name, a.type, a.currency, a.icon, a.color, a.opening_balance,
    a.opening_balance
...
all_account_json as (select coalesce(jsonb_agg(to_jsonb(x) order by x.name asc), '[]'::jsonb) as value from account_balances x),
...
select jsonb_build_object(
  ...
  'all_accounts', (select value from all_account_json),
