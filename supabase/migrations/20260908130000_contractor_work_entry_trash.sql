...
create policy contractor_work_entry_trash_select_own
on public.contractor_work_entry_trash
for select to authenticated
using (profile_id = auth.uid());

drop policy if exists contractor_work_entry_trash_delete_own on public.contractor_work_entry_trash;
create policy contractor_work_entry_trash_delete_own
on public.contractor_work_entry_trash
...
begin
  ...
  on conflict (id) do update set
    ...
    commission_per_piece = excluded.commission_per_piece,
    actual_rate_per_piece = excluded.actual_rate_per_piece,
...
create trigger contractor_work_entry_before_delete_trash
before delete on public.contractor_work_entries
for each row execute function public.move_contractor_work_entry_to_trash();

create index if not exists contractor_work_entry_trash_profile_deleted_idx
on public.contractor_work_entry_trash(profile_id, deleted_at desc);
