...
create or replace function private.worker_team_work_member(p_team_id bigint)
returns boolean language sql stable security definer set search_path = '' as $$
  select exists(select 1 from public.contractor_team_members m where m.team_id=p_team_id and m.profile_id=(select auth.uid()));
$$;
...
begin
  if new.id is distinct from old.id or new.team_id is distinct from old.team_id or new.worker_profile_id is distinct from old.worker_profile_id or new.occurred_at is distinct from old.occurred_at or new.created_at is distinct from old.created_at then
    raise exception 'immutable team work entry fields cannot be changed';
  end if;
...
begin
  insert into public.worker_team_work_entry_versions(work_entry_id,revision_no,item_name,size,quantity,rate,total,special_note,recorded_at,changed_by)
  values(new.id,coalesce((select max(v.revision_no) from public.worker_team_work_entry_versions v where v.work_entry_id=new.id),0)+1,new.item_name,new.size,new.quantity,new.rate,new.total,new.special_note,now(),(select auth.uid()));
  return new;
end;
$$;
...
create or replace function public.get_worker_team_work_context(p_team_number bigint)
returns table(team_id bigint,team_number bigint,team_name text,team_purpose text,leader_profile_id uuid,leader_display_name text,leader_username text,leader_avatar_url text,worker_profile_id uuid)
language sql security definer stable set search_path = '' as $$
  select t.id,t.team_number,t.name,t.purpose,t.leader_profile_id,p.display_name,p.username,p.avatar_url,wp.id
...
create or replace function public.get_worker_team_work_totals(p_team_number bigint,p_day_start timestamptz,p_day_end timestamptz,p_week_start timestamptz,p_week_end timestamptz,p_month_start timestamptz,p_month_end timestamptz)
returns table(daily_total numeric(24,4),weekly_total numeric(24,4),monthly_total numeric(24,4),lifetime_total numeric(24,4))
language sql security definer stable set search_path = '' as $$
  ...
  where t.team_number=p_team_number and e.lifecycle_state='active' and e.worker_profile_id in(select wp.id from public.worker_profiles wp where wp.profile_id=(select auth.uid())) and exists(select 1 from public.contractor_team_members m where m.team_id=e.team_id and m.profile_id=(select auth.uid()));
$$;
...
create or replace function public.trash_worker_team_work_entry(p_entry_id uuid)
returns boolean language plpgsql security definer set search_path = '' as $$
begin
  update public.worker_team_work_entries e set lifecycle_state='trashed',updated_at=now()
  where e.id=p_entry_id and e.worker_profile_id in(select wp.id from public.worker_profiles wp where wp.profile_id=(select auth.uid())) and exists(select 1 from public.contractor_team_members m where m.team_id=e.team_id and m.profile_id=(select auth.uid())) and e.lifecycle_state='active';
  return found;
end;
$$;
