...
Deno.serve(async (req) => {
  ...
  for (const reminder of reminders ?? []) {
    ...
    if (!liveReminder || !liveEntry || !liveReminder.enabled || liveReminder.status !== 'processing' || (liveReminder.reminder_kind === 'todo' && liveEntry.completed)) {
      await admin.from('worker_diary_reminders').update({ enabled: false, status: 'cancelled', claimed_at: null, updated_at: new Date().toISOString() }).eq('id', reminder.id);
      cancelled++; results.push({ id: reminder.id, status: 'cancelled' }); continue;
    }
    ...
    const title = liveEntry.title || (reminder.reminder_kind === 'event' ? 'Personal Diary Event' : 'Personal Diary To-do');
    let delivered = false, lastError = '';
    for (const subscription of subscriptions) {
      try {
        await webpush.sendNotification({ endpoint: subscription.endpoint, keys: { p256dh: subscription.p256dh, auth: subscription.auth } }, JSON.stringify({ title, body: `Reminder: ${title}`, tag: `diary-${reminder.id}`, reminderId: reminder.id, url: '/work/diary' }), { TTL: 300, urgency: 'high', topic: reminder.id.replace(/-/g, '').slice(0, 32) });
        delivered = true;
      } catch (error) {
        lastError = error instanceof Error ? error.message : 'Web Push delivery failed.';
        const statusCode = (error as { statusCode?: number }).statusCode;
        if (statusCode === 404 || statusCode === 410) await admin.from('worker_diary_push_subscriptions').delete().eq('id', subscription.id);
      }
    }
    if (delivered) { await admin.from('worker_diary_reminders').update({ status: 'sent', sent_at: new Date().toISOString(), claimed_at: null, last_error: null, updated_at: new Date().toISOString() }).eq('id', reminder.id); sent++; results.push({ id: reminder.id, status: 'sent' }); }
    else { await admin.from('worker_diary_reminders').update({ status: 'failed', claimed_at: null, last_error: lastError || 'Web Push delivery failed.', updated_at: new Date().toISOString() }).eq('id', reminder.id); failed++; results.push({ id: reminder.id, status: 'failed' }); }
  }
