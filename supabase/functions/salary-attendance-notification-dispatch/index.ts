...
Deno.serve(async req => {
  try {
    ...
    console.log('salary-attendance-dispatch tick', { localTime, attendanceDate });
    ...
    if (policyError) {
      console.error('salary-attendance policy lookup failed', policyError);
    ...
    for (const policy of policies ?? []) {
      const workerProfiles = policy.worker_profiles as unknown as { profile_id?: string } | { profile_id?: string }[] | null;
      const profileId = Array.isArray(workerProfiles) ? workerProfiles[0]?.profile_id : workerProfiles?.profile_id;
      if (!profileId) { failed++; results.push({ worker_profile_id: policy.worker_profile_id, status: 'profile_missing' }); continue; }
      ...
      for (const subscription of subscriptions) {
        try {
          await webpush.sendNotification(
            ...
            JSON.stringify({ title: 'Attendance Reminder', body: "Today's attendance is not marked yet. Tap to mark attendance.", tag: `attendance-${profileId}-${attendanceDate}`, url: '/work/finance', notificationId: notification.id, notificationType: 'attendance_reminder' }),
            { TTL: 300, urgency: 'high', topic: `attendance-${profileId}-${attendanceDate}`.replace(/-/g, '').slice(0, 32) }
          );
          delivered = true;
        } catch (error) {
          const statusCode = (error as { statusCode?: number }).statusCode;
          console.warn('attendance push delivery failed', { worker_profile_id: policy.worker_profile_id, statusCode });
    ...
    console.error('salary-attendance dispatch failed', error);
