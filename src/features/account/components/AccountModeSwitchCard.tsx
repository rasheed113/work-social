  ...
  return <>
    <section style={cardStyle} aria-labelledby="account-mode-switch-title">
      <button type="button" onClick={() => { setError(''); setOpen(true); }} disabled={switching} aria-haspopup="dialog" aria-expanded={open} style={{ width: '100%', border: 0, padding: 0, background: 'transparent', textAlign: 'left', cursor: switching ? 'wait' : 'pointer', font: 'inherit' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8 }}><div><h2 id="account-mode-switch-title" style={{ margin: 0, fontSize: 14, color: '#172033', letterSpacing: '-.015em' }}>Account / Work Mode</h2><p style={{ margin: '2px 0 0', color: '#64748b', fontSize: 10.5, lineHeight: 1.35 }}>Switch between your available account settings.</p></div><span style={{ flex: '0 0 auto', padding: '4px 7px', borderRadius: 999, fontSize: 8.5, fontWeight: 950, letterSpacing: '.07em', color: currentMode === 'contractor' ? '#047857' : '#4338ca', background: currentMode === 'contractor' ? 'rgba(16,185,129,.10)' : 'rgba(99,102,241,.10)', border: '1px solid rgba(99,102,241,.10)', boxShadow: 'inset 0 1px 0 rgba(255,255,255,.9)' }}>{current.badge}</span></div>
        <div style={{ marginTop: 8, minHeight: 38, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10, padding: '0 11px', borderRadius: 10, border: '1px solid rgba(99,102,241,.18)', background: 'linear-gradient(145deg,rgba(255,255,255,.95),rgba(238,242,255,.72))', boxShadow: 'inset 0 1px 0 rgba(255,255,255,.95), 0 7px 16px rgba(79,70,229,.07)' }}><span style={{ color: '#172033', fontSize: 12, fontWeight: 800 }}>{current.title}</span><span aria-hidden="true" style={{ color: '#6366f1', fontSize: 16, fontWeight: 900 }}>⌄</span></div>
      </button>
