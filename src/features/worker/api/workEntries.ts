...
import { normalizeWorkerWorkTotals } from '../logic/workEntryCalculations';

const WORK_ENTRY_COLUMNS =
  'id, worker_profile_id, work_context, item_name, size, quantity, rate, total, special_note, occurred_at, created_at, updated_at';
...
export async function listWorkerWorkEntries(limit: number, offset: number) {
  const { data, error, count } = await supabase
    .from('work_entries')
    .select(WORK_ENTRY_COLUMNS, { count: 'exact' })
    .order('occurred_at', { ascending: false })
    .order('id', { ascending: false })
    .range(offset, offset + limit - 1)
    .returns<WorkEntryRow[]>();
...
export async function getWorkerWorkEntry(entryId: string) {
  ...
  return {
    data: result.data ? normalizeEntry(result.data) : null,
    error: result.error,
  };
}
...
export async function getWorkerWorkEntryVersions(entryId: string) {
  const result = await supabase
    .from('work_entry_versions')
    .select('id, work_entry_id, revision_no, item_name, size, quantity, rate, total, special_note, recorded_at, changed_by')
    .eq('work_entry_id', entryId)
    .order('revision_no', { ascending: true })
    .returns<WorkEntryVersionRow[]>();

  return {
    data: result.data?.map(normalizeVersion) ?? [],
    error: result.error,
  };
}
...
export async function hideWorkerWorkEntry(entryId: string, profileId: string) {
  return supabase
    .from('work_entry_hidden_for')
    .insert({ work_entry_id: entryId, profile_id: profileId });
}

export async function getWorkerWorkTotals(bounds: ReturnType<typeof import('../logic/workEntryCalculations').getWorkerWorkPeriodBounds>) {
  const result = await supabase.rpc('get_worker_work_totals', {
    p_day_start: bounds.dayStart,
    p_day_end: bounds.dayEnd,
  ...
  return {
    data: normalizeWorkerWorkTotals(row as Partial<WorkerWorkTotals> | null | undefined),
    error: result.error,
  };
}
