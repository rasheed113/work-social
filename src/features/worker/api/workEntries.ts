...
export async function listWorkerWorkEntries(limit: number, cursor: WorkHistoryCursor | null = null, period: WorkHistoryPeriod = 'lifetime', selectedBounds: WorkHistoryPeriodBounds | null = null) {
  ...
  query = periodQuery(query, period, selectedBounds);
...
export async function listWorkerTrash(limit = 50) { const result = await supabase.from('work_entries').select(WORK_ENTRY_COLUMNS).eq('lifecycle_state', 'trashed').order('updated_at', { ascending: false }).order('id', { ascending: false }).limit(limit).returns<WorkEntryRow[]>(); return { data: result.data?.map(normalizeEntry) ?? [], error: result.error }; }
...
export async function createWorkerWorkEntry(input: WorkEntryInput) { const id = input.id ?? crypto.randomUUID(); const normalizedSize = normalizeWorkEntrySizes(input.size); const payload = { id, worker_profile_id: input.worker_profile_id, work_context: 'my_work' as const, item_name: input.item_name.trim(), size: normalizedSize.length ? normalizedSize : null, quantity: canonicalizeWorkDecimal(input.quantity), rate: canonicalizeWorkDecimal(input.rate), special_note: input.special_note?.trim() || null }; const result = await supabase.from('work_entries').insert(payload).select(WORK_ENTRY_COLUMNS).maybeSingle<WorkEntryRow>(); if (!result.error) return { data: result.data ? normalizeEntry(result.data) : null, error: null }; if (result.error.code !== '23505') return { data: null, error: result.error }; const existing = await getWorkerWorkEntry(id); if (existing.error || !existing.data) return { data: null, error: result.error }; const samePayload = existing.data.worker_profile_id === payload.worker_profile_id && existing.data.work_context === payload.work_context && existing.data.item_name === payload.item_name && JSON.stringify(existing.data.size) === JSON.stringify(payload.size) && existing.data.quantity === payload.quantity && existing.data.rate === payload.rate && existing.data.special_note === payload.special_note; return samePayload ? { data: existing.data, error: null } : { data: null, error: new Error('A Work Entry with this identity already exists with different values.') }; }
