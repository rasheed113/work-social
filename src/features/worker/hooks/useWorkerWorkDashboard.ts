...
const EMPTY_TOTALS: WorkerWorkTotals = {
  daily_personal_total: '0', daily_team_total: '0', daily_total: '0', weekly_personal_total: '0', weekly_team_total: '0', weekly_total: '0',
  monthly_personal_total: '0', monthly_team_total: '0', monthly_total: '0', lifetime_personal_total: '0', lifetime_team_total: '0', lifetime_total: '0',
};
...
export function useWorkerWorkDashboard(profileId: string) {
  ...
  const load = useCallback(async () => {
    ...
    if (workerResult.error) { setError(workerResult.error.message); setWorkerProfileId(null); setTotals(EMPTY_TOTALS); setLoading(false); return; }
    const nextWorkerProfileId = workerResult.data?.id ?? null; setWorkerProfileId(nextWorkerProfileId);
    if (!nextWorkerProfileId) { setTotals(EMPTY_TOTALS); setLoading(false); return; }
    const bounds = getWorkerWorkPeriodBounds(); const totalsResult = await getWorkerWorkTotals(bounds);
  ...
  const createEntry = useCallback(async (input: Omit<WorkEntryInput, 'worker_profile_id'>) => {
    ...
    setSaving(false); return result;
  }, [load, workerProfileId]);

  const periodLabels = useMemo(() => {
    const now = new Date(); const bounds = getWorkerWorkPeriodBounds(now);
    return { day: now.toLocaleDateString(undefined, { month: 'short', day: 'numeric' }), week: `Week of ${new Date(bounds.weekStart).toLocaleDateString(undefined, { month: 'short', day: 'numeric' })}`, month: now.toLocaleDateString(undefined, { month: 'long', year: 'numeric' }) };
  }, []);
