...
export function useWorkerWorkDashboard(profileId: string) {
  ...
  const [totals, setTotals] = useState<WorkerWorkTotals>({
    daily_total: 0,
    weekly_total: 0,
    monthly_total: 0,
    lifetime_total: 0,
  ...
  const load = useCallback(async () => {
    ...
    if (workerResult.error) {
      ...
      setEntries([]);
      setTotals({ daily_total: 0, weekly_total: 0, monthly_total: 0, lifetime_total: 0 });
      setLoading(false);
      return;
    }
    ...
    const [entriesResult, totalsResult] = await Promise.all([
      listWorkerWorkEntries(RECENT_LIMIT, 0),
      getWorkerWorkTotals(bounds),
    ]);

    if (entriesResult.error) setError(entriesResult.error.message);
    else setEntries(entriesResult.data);

    if (totalsResult.error) setError((current) => current ?? totalsResult.error.message);
    else setTotals(totalsResult.data);
  ...
  const periodLabels = useMemo(() => {
    ...
    return {
      ...
      week: `Week of ${getWorkerWorkPeriodBounds(now).weekStart.slice(0, 10)}`,
