  ...
  return <main style={{ ...shell, display: 'flex', flexDirection: 'column', background: 'linear-gradient(180deg,#f8fafc 0%,#f1f5f9 100%)' }}>
    ...
    <section style={{ ...dashboardStyle('salary_overview'), display: hiddenDashboardCards.includes('salary_overview') ? 'none' : 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(250px,1fr))', gap: 14, marginBottom: 16 }}>
      ...
      <InfoCard title="Allowance" icon="＋">
        {allowances.length === 0 ? <button type="button" onClick={() => navigate('/work/finance?setup=1&focus=allowances')} style={{ width: '100%', border: 0, background: 'transparent', padding: 0, textAlign: 'left', cursor: 'pointer', font: 'inherit' }}><div style={{ padding: 13, borderRadius: 14, background: '#fffbeb', border: '1px solid #fde68a' }}><div style={{ fontSize: 11, color: '#b45309', fontWeight: 900, letterSpacing: '.08em', textTransform: 'uppercase' }}>Not set</div><div style={{ marginTop: 5, fontSize: 24, fontWeight: 950 }}>Configure allowances</div><p style={{ margin: '8px 0 0', color: '#64748b', fontSize: 13 }}>Tap to open Salary Settings and set your allowance rules.</p><div style={{ marginTop: 10, color: '#b45309', fontWeight: 900 }}>Set →</div></div></button> : <div>
          <button type="button" onClick={() => setAllowanceOpen(open => !open)} aria-expanded={allowanceOpen} style={{ width: '100%', border: 0, background: 'transparent', padding: 0, textAlign: 'left', cursor: 'pointer', font: 'inherit' }}>
            <div style={{ padding: 13, borderRadius: 14, background: '#ecfdf5', border: '1px solid #bbf7d0' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12 }}><div><div style={{ fontSize: 11, color: '#047857', fontWeight: 900, letterSpacing: '.08em', textTransform: 'uppercase' }}>Set</div><div style={{ marginTop: 5, fontSize: 24, fontWeight: 950 }}>{money(allowanceTotal, currency)}</div><p style={{ margin: '6px 0 0', color: '#64748b', fontSize: 13 }}>{allowances.length} allowance{allowances.length === 1 ? '' : 's'} configured</p></div><span aria-hidden="true" style={{ fontSize: 22, color: '#047857', fontWeight: 900 }}>{allowanceOpen ? '⌃' : '⌄'}</span></div>
            </div>
          </button>
          {allowanceOpen && <div style={{ marginTop: 10, display: 'grid', gap: 8 }}>
            {allowances.map(row => <div key={row.id} style={{ padding: 11, borderRadius: 12, background: '#f8fafc', border: '1px solid #eef2f7' }}><div style={{ display: 'flex', justifyContent: 'space-between', gap: 10, alignItems: 'flex-start' }}><div><strong style={{ fontSize: 13 }}>{row.allowance_type}</strong><div style={{ marginTop: 3, color: '#64748b', fontSize: 11 }}>{row.frequency === 'monthly' ? 'Monthly' : 'Other'} · {allowanceRuleLabel(row)}</div></div><strong style={{ fontSize: 13 }}>{money(Number(row.amount), currency)}</strong></div>{row.rule_note && row.eligibility_rule !== 'custom' && <div style={{ marginTop: 7, color: '#64748b', fontSize: 11, lineHeight: 1.4 }}>{row.rule_note}</div>}</div>)}
            <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: 2 }}>{allowanceSettingsButton}</div>
          </div>}
        </div>}
      </InfoCard>
