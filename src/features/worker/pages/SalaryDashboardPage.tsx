...
export function SalaryDashboardPage({ profileId }: { profileId: string }) {
  ...
  return <main style={{ ...shell, background: 'linear-gradient(180deg,#f8fafc 0%,#f1f5f9 100%)' }}>
    ...
    <section style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(250px,1fr))', gap: 14, marginBottom: 16 }}>
      ...
      <InfoCard title="Bonus" icon="★"><div style={{ fontSize: 24, fontWeight: 950 }}>{money(summary?.bonus_amount ?? 0, currency)}</div><p style={{ margin: '4px 0 0', color: '#64748b', fontSize: 13 }}>Bonuses recorded this month</p><div style={{ marginTop: 14, padding: 10, borderRadius: 12, background: '#f8fafc', color: '#475569', fontSize: 13 }}>{policy ? `Salary cycle: ${salaryTypeLabel}` : 'Complete salary setup to configure your policy.'}</div></InfoCard>
    </section>
    ...
    <section style={{ ...panel, padding: 18 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, alignItems: 'center', marginBottom: 12 }}><div><div style={{ color: '#64748b', fontSize: 11, fontWeight: 900, letterSpacing: '.1em' }}>MONTHLY DETAIL</div><h2 style={{ margin: '5px 0' }}>{title}</h2></div><span style={{ padding: '6px 9px', borderRadius: 999, background: '#f8fafc', color: '#475569', fontSize: 12, fontWeight: 800 }}>{salaryTypeLabel}</span></div>
