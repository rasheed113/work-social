...
import { getSalaryMonthSummary, getSalaryPolicy, saveAttendance, saveOvertime } from '../api/salary';
...
export function SalaryDashboardPage({ profileId }: { profileId: string }) {
  ...
  useEffect(() => { if (!workerProfile) return; void (async () => { const [p, s] = await Promise.all([getSalaryPolicy(workerProfile.id), getSalaryMonthSummary(month)]); setPolicy(p.data); setSummary(s.data); })(); }, [workerProfile, month]);
  ...
  const saveAttendanceEntry = async () => { setStatus(''); const { error } = await saveAttendance(workerProfile.id, attendanceDate, attendance); setStatus(error ? error.message : 'Attendance saved.'); if (!error) { const refreshed = await getSalaryMonthSummary(month); setSummary(refreshed.data); } };
  const saveOvertimeEntry = async () => {
    const hours = Number(overtimeHours); if (!Number.isFinite(hours) || hours < 0) { setStatus('Enter valid overtime hours.'); return; }
    if (!policy?.working_hours) { setStatus('Working hours are not configured, so an overtime hourly basis cannot be invented.'); return; }
    const dailyRate = policy.salary_type === 'daily' ? policy.salary_amount : policy.salary_type === 'weekly' ? policy.salary_amount / 7 : policy.salary_type === '15_days' ? policy.salary_amount / 15 : policy.salary_amount / 30;
    const hourlyRate = dailyRate / policy.working_hours;
    const amount = hourlyRate * policy.overtime_multiplier * hours;
    const { error } = await saveOvertime(workerProfile.id, overtimeDate, hours, policy.overtime_multiplier, hourlyRate, amount); setStatus(error ? error.message : 'Overtime saved.'); if (!error) { setOvertimeHours(''); const refreshed = await getSalaryMonthSummary(month); setSummary(refreshed.data); }
  };
