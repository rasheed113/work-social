...
export function SalaryDashboardPage({ profileId }: { profileId: string }) {
  ...
  const refresh = async () => { const [p, s] = await Promise.all([getSalaryPolicy(profileId), getSalaryMonthSummary(month)]); setPolicy(p.data); setSummary(s.data); if (s.error) setStatus(s.error.message); };
