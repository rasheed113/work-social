...
import { WorkerTeamReportNotice } from '../components/WorkerTeamReportNotice';
...
import { WorkerTeamReportedEntryPage } from './WorkerTeamReportedEntryPage';
...
export function WorkerWorkHousePage() {
  ...
  if (/^\/work\/team-work\/\d+(?:\/|$)/.test(window.location.pathname)) {
    ...
    if (new URLSearchParams(window.location.search).has('report_entry')) return <WorkerTeamReportedEntryPage />;
    return <TeamDashboardWorkBoundary><><WorkerTeamReportNotice /><WorkerTeamDashboardWorkPageV2 /></></TeamDashboardWorkBoundary>;
  }
