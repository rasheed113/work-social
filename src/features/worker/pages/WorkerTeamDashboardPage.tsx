...
type TeamView = 'dashboard' | 'finance' | 'settings';
...
function navPath(teamNumber: number, view: TeamView) {
  return view === 'dashboard'
    ? `/work/team-work/${teamNumber}`
    : `/work/team-work/${teamNumber}/${view}`;
}

export function WorkerTeamDashboardPage() {
  const teamNumber = teamNumberFromPath();
  const view: TeamView = window.location.pathname.endsWith('/finance') ? 'finance' : window.location.pathname.endsWith('/settings') ? 'settings' : 'dashboard';
  ...
  const title = view === 'dashboard' ? 'Team Dashboard' : view === 'finance' ? 'Team Finance' : 'Team Settings';
  const subtitle = view === 'dashboard'
    ? 'A dedicated Team Work workspace. Personal work stays outside this context.'
    : view === 'finance'
  ...
  if (loading) return <main className="team-dashboard-page"><div className="td-state"><strong>Opening Team Work…</strong><p>Checking your approved membership for this team.</p></div></main>;
  if (error || !team) return <main className="team-dashboard-page"><div className="td-state td-error"><strong>Team Work unavailable</strong><p>{error || 'This team could not be opened.'}</p><button type="button" onClick={() => navigate('/work/team-work')}>← Back to My Teams</button></div></main>;
  ...
  return (
    <main className="team-dashboard-page">
      ...
      <div className="td-shell">
        ...
        <nav className="td-nav" aria-label="Team Work navigation">
          <button type="button" className={view === 'dashboard' ? 'active' : ''} onClick={() => navigate(navPath(team.team_number, 'dashboard'))}>⌂ Dashboard</button>
          <button type="button" className={view === 'finance' ? 'active' : ''} onClick={() => navigate(navPath(team.team_number, 'finance'))}>¤ Finance</button>
          <button type="button" className={view === 'settings' ? 'active' : ''} onClick={() => navigate(navPath(team.team_number, 'settings'))}>⚙ Settings</button>
        </nav>
        <section className="td-panel">
          <h2>{title}</h2>
          <p>{view === 'dashboard' ? 'This is the dedicated Team Work foundation. Personal My Work data is not displayed or queried into this workspace.' : view === 'finance' ? 'Team Finance is a separate context. It will receive Team Finance data only; personal finance records remain outside this workspace.' : 'Team Settings is a separate context for this team. Personal account settings remain outside this workspace.'}</p>
          ...
          {view === 'dashboard' && <div className="td-empty">No Team Work assignments or Team totals are invented in this foundation. Real Team Work distribution can be attached here in the next slice.</div>}
          {view === 'finance' && <div className="td-empty">Team Finance foundation is ready for real team-level finance data. Nothing from Personal Finance is copied here.</div>}
          {view === 'settings' && <div className="td-empty">Team Settings foundation is ready for team-specific controls without changing Personal Settings.</div>}
        </section>
      </div>
    </main>
  );
