...
export function WorkerWorkHistoryPage() {
  ...
  const periodHistory = useWorkerWorkPeriodHistory(period === 'lifetime' ? 'day' : period);

  useEffect(() => {
    if (!session.profileId) return;
    let active = true; setTotalsLoading(true);
    void getWorkerWorkTotals(getWorkerWorkPeriodBounds()).then((result) => { if (!active) return; if (!result.error) setTotals(result.data); setTotalsLoading(false); });
    return () => { active = false; };
  }, [session.profileId]);

  const changePeriod = (next: WorkHistoryPeriod) => { setPeriod(next); navigate(next === 'lifetime' ? '/work/history' : `/work/history?period=${next}`); };
  ...
  const showingPeriods = period !== 'lifetime';
  return (
    <main style={{ width: '100%', maxWidth: 900, margin: '0 auto', padding: '24px 14px 112px', boxSizing: 'border-box' }}>
      <header style={{ marginBottom: 18 }}>
        ...
        <p style={{ margin: '7px 0 0', color: '#64748b', lineHeight: 1.5 }}>{showingPeriods ? `Real ${period} periods, newest first. Five periods load initially, then ten older periods per request.` : 'Real persisted Work Entries, newest first. The list loads five initially, then ten more per request.'}</p>
      </header>
      ...
      <section style={{ ...cardStyle, marginBottom: 14 }} aria-label="Lifetime Grand Total">
        ...
        <div style={{ marginTop: 5, color: '#64748b', fontSize: 12 }}>Authoritative persisted total; it is independent from history page size.</div>
      </section>
      ...
      <section aria-labelledby="worker-history-list-heading">
        <h2 id="worker-history-list-heading" style={{ margin: '0 0 10px', fontSize: 19 }}>{showingPeriods ? `${period[0].toUpperCase()}${period.slice(1)} Periods` : 'Work Entries'}</h2>
        {showingPeriods ? (
          periodHistory.actionError ? <p role="alert" style={{ ...cardStyle, color: '#b91c1c', fontSize: 13, fontWeight: 700 }}>{periodHistory.actionError}</p> : periodHistory.loading ? <section style={cardStyle}><p style={{ margin: 0, color: '#64748b' }}>Loading {period} history…</p></section> : periodHistory.periods.length === 0 ? <section style={{ ...cardStyle, textAlign: 'center' }}><h3 style={{ margin: 0, fontSize: 17 }}>No {period} work history yet</h3><p style={{ margin: '7px 0 0', color: '#64748b', fontSize: 13 }}>Periods appear only when real persisted Worker Work exists.</p></section> : <><WorkerWorkPeriodList periods={periodHistory.periods} period={period} />{periodHistory.hasMore && <button type="button" onClick={() => void periodHistory.loadMore()} disabled={periodHistory.loadingMore} style={{ display: 'block', width: '100%', marginTop: 12, minHeight: 44, borderRadius: 12, fontWeight: 800, cursor: periodHistory.loadingMore ? 'not-allowed' : 'pointer' }}>{periodHistory.loadingMore ? 'Loading more…' : 'More'}</button>}</>
        ) : (
          history.actionError ? <p role="alert" style={{ ...cardStyle, color: '#b91c1c', fontSize: 13, fontWeight: 700 }}>{history.actionError}</p> : history.loading ? <section style={cardStyle}><p style={{ margin: 0, color: '#64748b' }}>Loading Work History…</p></section> : history.entries.length === 0 ? <section style={{ ...cardStyle, textAlign: 'center' }}><h3 style={{ margin: 0, fontSize: 17 }}>No Work Entries yet</h3><p style={{ margin: '7px 0 0', color: '#64748b', fontSize: 13 }}>Persisted Work Entries will appear here when they exist.</p></section> : <><WorkerWorkEntryList entries={history.entries} onOpen={(entry) => void history.openDetails(entry)} />{history.hasMore && <button type="button" onClick={() => void history.loadMore()} disabled={history.loadingMore} style={{ display: 'block', width: '100%', marginTop: 12, minHeight: 44, borderRadius: 12, fontWeight: 800, cursor: history.loadingMore ? 'not-allowed' : 'pointer' }}>{history.loadingMore ? 'Loading more…' : 'More'}</button>}</>
        )}
      </section>

      {!showingPeriods && <WorkerWorkEntryDetails entry={history.selectedEntry} versions={history.versions} versionsLoading={history.versionsLoading} actionError={history.actionError} onClose={history.closeDetails} onEdit={editEntry} onTrash={trashEntry} />}
    </main>
  );
}
