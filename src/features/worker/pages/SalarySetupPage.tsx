...
export function SalarySetupPage({ profileId }: { profileId: string }) {
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    if (allowanceLossRule === 'threshold' && (!allowanceThreshold || Number(allowanceThreshold) < 1)) return setMessage('Enter the absence threshold for allowance loss.');
    ...
    const policy: SalaryPolicyInput = {
      ...
      working_hours: workingHours, overtime_multiplier: ot, sunday_paid: sundayPaid, holidays_paid: holidaysPaid,
      attendance_notification_time: notificationTime || null, pay_date: payDate ? Number(payDate) : null,
      salary_start_date: startDate, total_salary: totalSalary ? Number(totalSalary) : null,
      basic_salary: basicSalary ? Number(basicSalary) : null, attendance_allowance: attendanceAllowance ? Number(attendanceAllowance) : null,
    ...
    if (showBonus) {
      const bonusResult = await saveBonusPolicy(workerProfile.id, {
        frequency: bonusFrequency, expected_month_count: expectedMonths,
        amount_type: bonusAmountType, fixed_amount: bonusAmountType === 'fixed_amount' ? Number(bonusFixedAmount) : null,
  ...
  if (!privacyAccepted) return <main style={{ width: '100%', maxWidth: 680, margin: '0 auto', padding: '34px 14px 112px', boxSizing: 'border-box' }}>
    <section style={{ ...card, textAlign: 'center', padding: 28 }}>
      ...
      <p style={{ color: '#475569', lineHeight: 1.65, margin: 0 }}>Your salary information is used to maintain your personal salary, attendance, overtime, bonus, and salary records in Work Social. It is not intended for public Social display or sharing.</p>
  ...
  if (saved) return <main style={{ width: '100%', maxWidth: 680, margin: '0 auto', padding: '34px 14px 112px', boxSizing: 'border-box' }}><section style={{ ...card, textAlign: 'center', padding: 30 }}>
    ...
    <p style={{ color: '#475569', lineHeight: 1.6 }}>Your salary policy{showBonus ? ' and bonus policy' : ''} has been saved. Work Social can now use these settings when calculating future salary and overtime records.</p>
  ...
  return <main style={{ width: '100%', maxWidth: 760, margin: '0 auto', padding: '24px 14px 112px', boxSizing: 'border-box' }}>
    ...
    <form onSubmit={submit} style={{ display: 'grid', gap: 14 }}>
      ...
      <section style={{ ...card, display: 'grid', gap: 14 }}><button type="button" onClick={() => setShowRules(v => !v)} style={{ border: 0, background: 'transparent', padding: 0, textAlign: 'left', cursor: 'pointer' }}><SectionTitle title="Allowances & Salary Rules" subtitle="Optional: only add rules that genuinely apply to your salary." open={showRules}/></button>{showRules && <div style={{ display: 'grid', gap: 12 }}>
        <div style={{ padding: 13, borderRadius: 13, background: '#f8fafc', ...hint }}>Allowance values are separate from absence deductions. For example, an attendance allowance can be lost under its own threshold while an absence deduction follows its own rule.</div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}><label style={label}>Attendance Allowance<input type="number" min="0" step="0.01" value={attendanceAllowance} onChange={e => setAttendanceAllowance(e.target.value)} style={input} placeholder="Optional"/></label><label style={label}>Other Allowance<input type="number" min="0" step="0.01" value={otherAllowance} onChange={e => setOtherAllowance(e.target.value)} style={input} placeholder="Optional"/></label></div>
        ...
        <label style={label}>Attendance Allowance Loss<select value={allowanceLossRule} onChange={e => setAllowanceLossRule(e.target.value as 'none'|'threshold')} style={input}><option value="none">No allowance loss</option><option value="threshold">Loss after X absences</option></select></label>
        {allowanceLossRule === 'threshold' && <label style={label}>Allowance Loss After X Absences<input type="number" min="1" step="1" value={allowanceThreshold} onChange={e => setAllowanceThreshold(e.target.value)} style={input}/></label>}
      ...
      <p style={{ ...hint, textAlign: 'center', margin: 0 }}>Your settings are saved to your personal Salary Person record and used for future calculations.</p>
    </form>
  </main>;
}
