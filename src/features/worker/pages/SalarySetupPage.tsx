...
function PremiumTimePicker({ value, onChange }: { value: string; onChange: (value: string) => void }) {
  const [open, setOpen] = useState(false);
  const parse = (time: string) => {
    if (!time) return { hour: '09', minute: '00', period: 'AM' as const };
    const [rawHour, rawMinute] = time.split(':').map(Number);
    const hour = rawHour % 12 || 12;
    return { hour: String(hour).padStart(2, '0'), minute: String(rawMinute).padStart(2, '0'), period: rawHour >= 12 ? 'PM' as const : 'AM' as const };
  };
  ...
  const openPicker = () => { const next = parse(value); setHour(next.hour); setMinute(next.minute); setPeriod(next.period); setOpen(true); };
  const choose = () => {
  ...
  const display = value ? (() => { const parsed = parse(value); return `${Number(parsed.hour)}:${parsed.minute} ${parsed.period}`; })() : 'Not set';
  const choice = (active: boolean): React.CSSProperties => ({ minHeight: 36, padding: '0 9px', borderRadius: 10, border: active ? '1.5px solid #6366f1' : '1px solid #dbe3ee', background: active ? 'linear-gradient(145deg,#eef2ff,#e0e7ff)' : '#fff', color: active ? '#3730a3' : '#334155', fontWeight: 900, cursor: 'pointer', fontSize: 12 });
  return <>
    <label style={label}>Attendance Notification Time
      <button type="button" onClick={openPicker} aria-haspopup="dialog" aria-expanded={open} style={{ ...input, minHeight: 42, display: 'flex', alignItems: 'center', justifyContent: 'space-between', cursor: 'pointer', textAlign: 'left', boxShadow: '0 3px 10px rgba(15,23,42,.04)' }}>
        <span style={{ color: value ? '#172033' : '#94a3b8', fontWeight: value ? 800 : 650 }}>{display}</span><span style={{ color: '#6366f1', fontWeight: 900 }}>⌄</span>
      </button>
      <span style={hint}>Leave empty to disable attendance reminders.</span>
    </label>
    {open && <div role="presentation" onMouseDown={event => { if (event.target === event.currentTarget) setOpen(false); }} style={{ position: 'fixed', inset: 0, zIndex: 1000, display: 'grid', placeItems: 'center', padding: 16, background: 'rgba(2,6,23,.58)', backdropFilter: 'blur(9px)' }}>
      <div role="dialog" aria-modal="true" aria-label="Attendance Notification Time" style={{ width: '100%', maxWidth: 340, borderRadius: 20, padding: 14, background: 'linear-gradient(145deg,#ffffff,#eef2ff)', border: '1px solid rgba(255,255,255,.9)', boxShadow: '0 26px 70px rgba(2,6,23,.3), inset 0 1px 0 #fff' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8, marginBottom: 10 }}><div><div style={{ color: '#6366f1', fontSize: 9, fontWeight: 900, letterSpacing: '.12em', textTransform: 'uppercase' }}>Reminder time</div><h3 style={{ margin: '3px 0 0', fontSize: 17, color: '#0f172a' }}>Attendance Notification</h3></div><button type="button" onClick={() => setOpen(false)} aria-label="Close" style={{ width: 32, height: 32, border: 0, borderRadius: '50%', background: '#e2e8f0', color: '#334155', fontSize: 17, cursor: 'pointer' }}>×</button></div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 7, marginBottom: 10 }}>
          <div><div style={{ ...hint, fontSize: 9.5, fontWeight: 900, marginBottom: 5 }}>HOUR</div><div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 5 }}>{Array.from({ length: 12 }, (_, index) => { const next = String(index + 1).padStart(2, '0'); return <button key={next} type="button" onClick={() => setHour(next)} style={choice(hour === next)}>{index + 1}</button>; })}</div></div>
          <div><div style={{ ...hint, fontSize: 9.5, fontWeight: 900, marginBottom: 5 }}>MINUTE</div><div style={{ display: 'grid', gridTemplateColumns: 'repeat(2,1fr)', gap: 5 }}>{['00','15','30','45'].map(next => <button key={next} type="button" onClick={() => setMinute(next)} style={choice(minute === next)}>{next}</button>)}</div></div>
          <div><div style={{ ...hint, fontSize: 9.5, fontWeight: 900, marginBottom: 5 }}>PERIOD</div><div style={{ display: 'grid', gap: 5 }}><button type="button" onClick={() => setPeriod('AM')} style={choice(period === 'AM')}>AM</button><button type="button" onClick={() => setPeriod('PM')} style={choice(period === 'PM')}>PM</button></div></div>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8, paddingTop: 9, borderTop: '1px solid #e2e8f0' }}><button type="button" onClick={() => { onChange(''); setOpen(false); }} style={{ minHeight: 34, padding: '0 10px', border: 0, borderRadius: 9, background: '#f1f5f9', color: '#64748b', fontWeight: 850, cursor: 'pointer', fontSize: 11.5 }}>Clear / Not set</button><div style={{ display: 'flex', gap: 6 }}><button type="button" onClick={() => setOpen(false)} style={{ minHeight: 34, padding: '0 10px', border: 0, borderRadius: 9, background: '#e2e8f0', color: '#334155', fontWeight: 850, cursor: 'pointer', fontSize: 11.5 }}>Cancel</button><button type="button" onClick={choose} style={{ minHeight: 34, padding: '0 12px', border: 0, borderRadius: 9, background: 'linear-gradient(145deg,#312e81,#0f172a)', color: '#fff', fontWeight: 900, cursor: 'pointer', fontSize: 11.5 }}>Set Time</button></div></div>
      </div>
    </div>}
  </>;
}
