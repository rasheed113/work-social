...
export function WorkerWorkEntryDetails({ entry, versions, versionsLoading, actionError, onClose, onEdit, onDeleteForMe }: WorkerWorkEntryDetailsProps) {
  ...
  const saveEdit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!itemName.trim() || !DECIMAL_RE.test(quantity) || quantity === '0' || !DECIMAL_RE.test(rate)) return;
    setSaving(true);
    const result = await onEdit(entry.id, { item_name: itemName, size: size.trim() || null, quantity, rate, special_note: specialNote.trim() || null });
    setSaving(false);
      ...
      <section role="dialog" aria-modal="true" aria-labelledby="work-entry-details-title" style={{ width: '100%', maxWidth: 620, maxHeight: 'calc(100dvh - 28px)', overflowY: 'auto', borderRadius: 22, background: '#fff', boxShadow: '0 24px 70px rgba(15,23,42,.3)', padding: 18, boxSizing: 'border-box' }}>
        ...
        {editing ? (
          <form onSubmit={saveEdit} style={{ display: 'grid', gap: 11, marginTop: 18 }}>
            ...
            <label style={{ display: 'grid', gap: 6, fontSize: 13, fontWeight: 700 }}>Size <span style={{ color: '#94a3b8', fontWeight: 500 }}>(optional)</span><input value={size} onChange={(event) => setSize(event.target.value)} maxLength={100} disabled={saving} placeholder="e.g. Small, XL, 40/120" /></label>
