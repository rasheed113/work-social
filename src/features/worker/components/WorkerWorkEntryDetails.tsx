...
interface WorkerWorkEntryDetailsProps {
  ...
  onTrash: (entryId: string) => Promise<{ error: unknown } | { error: null }>;
}
...
export function WorkerWorkEntryDetails({ entry, versions, versionsLoading, actionError, onClose, onEdit, onTrash }: WorkerWorkEntryDetailsProps) {
  ...
  const moveToTrash = async () => {
    setSaving(true);
    const result = await onTrash(entry.id);
    setSaving(false);
      ...
      <section role="dialog" aria-modal="true" aria-labelledby="work-entry-details-title" style={{ width: '100%', maxWidth: 620, maxHeight: 'calc(100dvh - 28px)', overflowY: 'auto', borderRadius: 22, background: '#fff', boxShadow: '0 24px 70px rgba(15,23,42,.3)', padding: 18, boxSizing: 'border-box' }}>
        ...
        {editing ? (
          ...
          <>
            ...
            {confirmDelete ? <section style={{ marginTop: 18, padding: 14, borderRadius: 14, background: '#fff7ed', border: '1px solid #fed7aa' }}><strong style={{ display: 'block' }}>⚠️ Delete for me?</strong><p style={{ margin: '6px 0 12px', color: '#7c2d12', fontSize: 12, lineHeight: 1.5 }}>Are you sure you want to delete this entry?</p><div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 9 }}><button type="button" onClick={() => setConfirmDelete(false)} disabled={saving} style={{ minHeight: 42, borderRadius: 11 }}>Cancel</button><button type="button" onClick={() => void moveToTrash()} disabled={saving} style={{ minHeight: 42, borderRadius: 11, fontWeight: 900 }}>{saving ? 'Moving…' : 'Yes'}</button></div></section> : <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 9, marginTop: 18 }}><button type="button" onClick={() => setEditing(true)} style={{ minHeight: 44, borderRadius: 12, fontWeight: 800 }}>Edit</button><button type="button" onClick={() => setConfirmDelete(true)} style={{ minHeight: 44, borderRadius: 12, fontWeight: 800 }}>Delete</button></div>}
          </>
        )}
      </section>
