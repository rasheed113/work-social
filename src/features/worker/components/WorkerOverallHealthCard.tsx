...
function buildHealthMap(container: Element, todayEntries: number, weekEntries: number, remaining: number) {
  ...
  const path = svgEl('path'); path.classList.add('wo-health-flow'); path.setAttribute('d', `M118 ${centerY} C190 ${centerY - 38},230 ${centerY + 38},310 ${centerY} S430 ${centerY - 38},502 ${centerY}`); path.setAttribute('fill', 'none'); path.setAttribute('stroke', 'rgba(255,255,255,.16)'); path.setAttribute('stroke-width', '2'); path.setAttribute('stroke-dasharray', '4 7'); svg.appendChild(path);
  nodes.forEach((node, index) => { const glow = svgEl('circle'); glow.classList.add('wo-health-glow'); glow.setAttribute('cx', String(node.x)); glow.setAttribute('cy', String(centerY)); glow.setAttribute('r', index === 1 ? '39' : '32'); glow.setAttribute('fill', node.color); glow.setAttribute('fill-opacity', index === 1 ? '.11' : '.055'); glow.setAttribute('filter', 'url(#health-map-glow)'); svg.appendChild(glow); const circle = svgEl('circle'); circle.classList.add('wo-health-node'); circle.setAttribute('cx', String(node.x)); circle.setAttribute('cy', String(centerY)); circle.setAttribute('r', index === 1 ? '29' : '25'); circle.setAttribute('fill', 'rgba(7,17,30,.97)'); circle.setAttribute('stroke', node.color); circle.setAttribute('stroke-width', index === 1 ? '2.5' : '2'); svg.appendChild(circle); addText(svg, node.label, node.x, centerY - 8, 7, '#9aaac0', '900', 'middle'); addText(svg, node.state, node.x, centerY + 3, 8, '#f5fbff', '950', 'middle'); addText(svg, node.value, node.x, centerY + 14, 6, node.color, '900', 'middle'); });
