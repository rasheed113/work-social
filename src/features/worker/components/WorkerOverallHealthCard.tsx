...
function buildHealthMap(container: Element, todayEntries: number, weekEntries: number, remaining: number) {
  ...
  const path = svgEl('path'); path.classList.add('wo-health-flow'); path.setAttribute('d', `M118 ${centerY} C190 ${centerY - 38},230 ${centerY + 38},310 ${centerY} S430 ${centerY - 38},502 ${centerY}`); path.setAttribute('fill', 'none'); path.setAttribute('stroke', 'rgba(255,255,255,.16)'); path.setAttribute('stroke-width', '2'); path.setAttribute('stroke-dasharray', '4 7'); const flowAnim = svgEl('animate'); flowAnim.setAttribute('attributeName','stroke-dashoffset'); flowAnim.setAttribute('from','0'); flowAnim.setAttribute('to','-33'); flowAnim.setAttribute('dur','3.2s'); flowAnim.setAttribute('repeatCount','indefinite'); path.appendChild(flowAnim); svg.appendChild(path);
  nodes.forEach((node, index) => { const glow = svgEl('circle'); glow.classList.add('wo-health-glow'); glow.setAttribute('cx', String(node.x)); glow.setAttribute('cy', String(centerY)); glow.setAttribute('r', index === 1 ? '39' : '32'); glow.setAttribute('fill', node.color); glow.setAttribute('fill-opacity', index === 1 ? '.11' : '.055'); glow.setAttribute('filter', 'url(#health-map-glow)'); const glowAnim = svgEl('animate'); glowAnim.setAttribute('attributeName','opacity'); glowAnim.setAttribute('values','.45;1;.45'); glowAnim.setAttribute('dur','2.8s'); glowAnim.setAttribute('repeatCount','indefinite'); glow.appendChild(glowAnim); svg.appendChild(glow); const circle = svgEl('circle'); circle.classList.add('wo-health-node'); circle.setAttribute('cx', String(node.x)); circle.setAttribute('cy', String(centerY)); circle.setAttribute('r', index === 1 ? '29' : '25'); circle.setAttribute('fill', 'rgba(7,17,30,.97)'); circle.setAttribute('stroke', node.color); circle.setAttribute('stroke-width', index === 1 ? '2.5' : '2'); const nodeAnim = svgEl('animate'); nodeAnim.setAttribute('attributeName','stroke-width'); nodeAnim.setAttribute('values', index === 1 ? '2.5;3.5;2.5' : '2;3;2'); nodeAnim.setAttribute('dur','2.8s'); nodeAnim.setAttribute('repeatCount','indefinite'); circle.appendChild(nodeAnim); svg.appendChild(circle); addText(svg, node.label, node.x, centerY - 8, 7, '#9aaac0', '900', 'middle'); addText(svg, node.state, node.x, centerY + 3, 8, '#f5fbff', '950', 'middle'); addText(svg, node.value, node.x, centerY + 14, 6, node.color, '900', 'middle'); });
...
function buildFinanceOrbit(container: Element) {
  ...
  addText(svg, 'LIVE FINANCIAL POSITION', cx, cy + 23, 6, '#a9b8cc', '850', 'middle');
  ...
  const receivedDot = svgEl('circle'); receivedDot.setAttribute('cx',String(cx+78)); receivedDot.setAttribute('cy',String(cy)); receivedDot.setAttribute('r','5'); receivedDot.setAttribute('fill','#ec4899'); receivedDot.setAttribute('filter','url(#finance-orbit-glow)'); receivedDot.style.transformOrigin=`${cx}px ${cy}px`; receivedDot.style.animation='woOrbitSpin 7s linear infinite'; const receivedAnim = svgEl('animateTransform'); receivedAnim.setAttribute('attributeName','transform'); receivedAnim.setAttribute('type','rotate'); receivedAnim.setAttribute('from',`0 ${cx} ${cy}`); receivedAnim.setAttribute('to',`360 ${cx} ${cy}`); receivedAnim.setAttribute('dur','7s'); receivedAnim.setAttribute('repeatCount','indefinite'); receivedDot.appendChild(receivedAnim); svg.appendChild(receivedDot);
  const earnedDot = svgEl('circle'); earnedDot.setAttribute('cx',String(cx-78)); earnedDot.setAttribute('cy',String(cy)); earnedDot.setAttribute('r','4'); earnedDot.setAttribute('fill','#22d3ee'); earnedDot.setAttribute('filter','url(#finance-orbit-glow)'); earnedDot.style.transformOrigin=`${cx}px ${cy}px`; earnedDot.style.animation='woOrbitSpinReverse 11s linear infinite'; const earnedAnim = svgEl('animateTransform'); earnedAnim.setAttribute('attributeName','transform'); earnedAnim.setAttribute('type','rotate'); earnedAnim.setAttribute('from',`0 ${cx} ${cy}`); earnedAnim.setAttribute('to',`-360 ${cx} ${cy}`); earnedAnim.setAttribute('dur','11s'); earnedAnim.setAttribute('repeatCount','indefinite'); earnedDot.appendChild(earnedAnim); svg.appendChild(earnedDot);

  const particles = [[cx-104,cy-48,'#22d3ee'],[cx+108,cy-31,'#3b82f6'],[cx+101,cy+48,'#8b5cf6'],[cx-92,cy+54,'#ec4899'],[cx+18,cy-103,'#67e8f9'],[cx-31,cy+104,'#a78bfa']];
  particles.forEach(([x,y,color],i)=>{const p=svgEl('circle'); p.setAttribute('cx',String(x)); p.setAttribute('cy',String(y)); p.setAttribute('r',i%2?'2.5':'2'); p.setAttribute('fill',String(color)); p.setAttribute('opacity','.85'); p.style.animation=`woEnergyPulse ${1.8+i*.23}s ease-in-out infinite alternate`; const particleAnim = svgEl('animate'); particleAnim.setAttribute('attributeName','opacity'); particleAnim.setAttribute('values','.25;.95;.25'); particleAnim.setAttribute('dur',`${1.8+i*.23}s`); particleAnim.setAttribute('repeatCount','indefinite'); p.appendChild(particleAnim); svg.appendChild(p);});
...
export function WorkerOverallHealthCard({ todayEntries, weekEntries, remaining }: Props) {
  ...
  return (
    <section className="wo-card wo-health-card">
      <style>{`.wo-health-card{position:relative;overflow:hidden;background:transparent!important;background-image:none!important;border-color:transparent!important;box-shadow:none!important}.wo-health-card:before,.wo-health-card:after{content:'';display:block!important;position:absolute;inset:-45%;z-index:0;pointer-events:none;background:conic-gradient(from 0deg at 50% 50%,transparent 0deg,rgba(34,211,238,.11) 32deg,transparent 72deg,rgba(124,58,237,.1) 148deg,transparent 205deg,rgba(236,72,153,.08) 266deg,transparent 330deg);animation:woHealthAmbient 12s linear infinite}.wo-health-card:after{inset:-20%;background:radial-gradient(circle at 50% 48%,rgba(34,211,238,.12),transparent 28%),radial-gradient(circle at 78% 22%,rgba(167,139,250,.1),transparent 25%);animation:woHealthBreathe 4.5s ease-in-out infinite;mix-blend-mode:screen}.wo-health-card>*{position:relative;z-index:1}.wo-health-energy{position:absolute!important;z-index:0!important;left:-35%;top:0;width:34%;height:100%;pointer-events:none;background:linear-gradient(90deg,transparent,rgba(103,232,249,.12),rgba(196,181,253,.08),transparent);filter:blur(10px);transform:skewX(-16deg);animation:woHealthSweep 5.5s ease-in-out infinite}.wo-health-map-anchor{display:block;height:180px;margin-top:12px}.wo-live-map-health{display:block;width:100%;height:auto;padding:7px;border:1px solid rgba(255,255,255,.08);border-radius:18px;background:radial-gradient(circle at 50% 50%,rgba(124,58,237,.09),rgba(255,255,255,.025) 45%,rgba(255,255,255,.012));box-shadow:inset 0 1px 0 rgba(255,255,255,.05),0 12px 28px rgba(0,0,0,.16)}.wo-live-finance-orbit{display:block;width:100%;height:auto;margin-top:10px;padding:7px;border:1px solid rgba(255,255,255,.1);border-radius:20px;background:radial-gradient(circle at 50% 48%,rgba(59,130,246,.10),rgba(124,58,237,.075) 35%,rgba(236,72,153,.045) 62%,rgba(255,255,255,.012) 100%);box-shadow:inset 0 1px 0 rgba(255,255,255,.07),0 18px 38px rgba(15,23,42,.2)}.wo-live-finance-orbit text{font-family:inherit;letter-spacing:.03em}@keyframes woHealthAmbient{to{transform:rotate(360deg)}}@keyframes woHealthBreathe{0%,100%{opacity:.45;transform:scale(.94)}50%{opacity:1;transform:scale(1.04)}}@keyframes woHealthSweep{0%{transform:translateX(0) skewX(-16deg);opacity:0}15%{opacity:.75}50%{opacity:.9}85%{opacity:.75}100%{transform:translateX(390%) skewX(-16deg);opacity:0}}@keyframes woOrbitSpin{from{transform:rotate(0deg)}to{transform:rotate(360deg)}}@keyframes woOrbitSpinReverse{from{transform:rotate(360deg)}to{transform:rotate(0deg)}}@keyframes woEnergyPulse{from{opacity:.25;transform:scale(.75)}to{opacity:1;transform:scale(1.35)}}@keyframes woHealthFlow{to{stroke-dashoffset:-33}}@keyframes woHealthGlow{0%,100%{opacity:.45}50%{opacity:1}}@keyframes woHealthNode{0%,100%{stroke-width:2}50%{stroke-width:3}}.wo-health-flow{animation:woHealthFlow 3.2s linear infinite}.wo-health-glow{animation:woHealthGlow 2.8s ease-in-out infinite}.wo-health-node{animation:woHealthNode 2.8s ease-in-out infinite}.wo-health-note{position:relative;z-index:1;margin-top:9px;padding:9px 10px;border:1px solid rgba(34,211,238,.1);border-left:3px solid #22d3ee;border-radius:11px;background:linear-gradient(90deg,rgba(34,211,238,.07),rgba(34,211,238,.02));color:#91a2bd;font-size:7px;line-height:1.5}.wo-health-grid{display:none}@media(max-width:400px){.wo-health-map-anchor{height:180px}.wo-live-finance-orbit{margin-top:8px}}`}</style>
      <div className="wo-health-energy" aria-hidden="true" />
