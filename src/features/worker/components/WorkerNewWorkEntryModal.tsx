...
export function WorkerNewWorkEntryModal({ open, saving, onClose, onSave }: WorkerNewWorkEntryModalProps) {
  ...
  const resetAndClose = () => {
    if (saving) return;
    setItemName(''); setSize(''); setQuantity(''); setRate(''); setSpecialNote(''); setValidationError(null); onClose();
  };
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const result = await onSave({ item_name: itemName, size, quantity: numericQuantity, rate: numericRate, special_note: specialNote.trim() || null });
    ...
    <div style={overlayStyle} role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) resetAndClose(); }}>
      <section role="dialog" aria-modal="true" aria-labelledby="new-work-entry-title" style={{ width: '100%', maxWidth: 560, maxHeight: 'calc(100dvh - 28px)', overflowY: 'auto', borderRadius: 22, background: '#fff', boxShadow: '0 24px 70px rgba(15,23,42,.3)', padding: 18, boxSizing: 'border-box' }}>
        ...
        <form onSubmit={submit} style={{ display: 'grid', gap: 12, marginTop: 18 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}><label style={{ display: 'grid', gap: 6, fontWeight: 700, fontSize: 13 }}>Pieces / Quantity<input type="number" inputMode="decimal" min="0.0001" step="0.0001" value={quantity} onChange={(event) => setQuantity(event.target.value)} disabled={saving} /></label><label style={{ display: 'grid', gap: 6, fontWeight: 700, fontSize: 13 }}>Rate<input type="number" inputMode="decimal" min="0" step="0.0001" value={rate} onChange={(event) => setRate(event.target.value)} disabled={saving} /></label></div>
          <div style={{ padding: 14, borderRadius: 14, background: 'linear-gradient(145deg,#eff6ff,#ecfeff)', border: '1px solid #bae6fd' }}><div style={{ color: '#475569', fontSize: 11, fontWeight: 800, textTransform: 'uppercase', letterSpacing: '.08em' }}>Live Total</div><div style={{ marginTop: 4, fontSize: 28, fontWeight: 950, letterSpacing: '-.04em' }}>{total.toLocaleString(undefined, { maximumFractionDigits: 4 })}</div><div style={{ marginTop: 3, color: '#64748b', fontSize: 11 }}>Quantity × Rate</div></div>
