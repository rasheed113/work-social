...
export function WorkerNewWorkEntryModal({ open, saving, onClose, onSave }: WorkerNewWorkEntryModalProps) {
  ...
  useEffect(() => {
    if (sizePickerOpen && customSizeInputRef.current) customSizeInputRef.current.focus();
  }, [sizePickerOpen]);
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (normalizedSize.length > 100) return setValidationError('Size must be 100 characters or fewer.');
    setValidationError(null);
    const result = await onSave({ id: entryId, item_name: itemName, size: normalizedSize || null, quantity, rate, special_note: specialNote.trim() || null });
      ...
      <section role="dialog" aria-modal="true" aria-labelledby="new-work-entry-title" style={{ width: '100%', maxWidth: 560, maxHeight: 'calc(100dvh - 28px)', overflowY: 'auto', borderRadius: 22, background: '#fff', boxShadow: '0 24px 70px rgba(15,23,42,.3)', padding: 18, boxSizing: 'border-box' }}>
        ...
        <form onSubmit={submit} style={{ display: 'grid', gap: 12, marginTop: 18 }}>
          ...
          <div style={{ display: 'grid', gap: 6 }}>
            ...
            <button id="work-entry-size-trigger" ref={sizeTriggerRef} type="button" disabled={saving} onClick={() => setSizePickerOpen(true)} aria-haspopup="dialog" aria-expanded={sizePickerOpen} style={{ minHeight: 52, width: '100%', padding: '0 14px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, borderRadius: 15, border: '1px solid #cbd5e1', background: '#fff', color: size ? '#0f172a' : '#94a3b8', font: 'inherit', fontWeight: 750, textAlign: 'left', cursor: saving ? 'not-allowed' : 'pointer', boxShadow: '0 2px 8px rgba(15,23,42,.04)' }}>
              <span style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{size || 'Optional — Select size'}</span>
        ...
        <div className="work-size-picker-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) closeSizePicker(); }}>
          <section className="work-size-picker-panel" role="dialog" aria-modal="true" aria-labelledby="work-size-picker-title">
            ...
            <header style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
              <div><h3 id="work-size-picker-title" style={{ margin: 0, fontSize: 20, letterSpacing: '-.025em' }}>Choose a size</h3><p style={{ margin: '4px 0 0', color: '#64748b', fontSize: 12 }}>Optional · pick a shortcut or enter your own.</p></div>
            ...
            <div style={{ display: 'grid', gap: 8, marginTop: 16 }}>
              <button className="work-size-option" type="button" data-selected={!size} onClick={clearSize}>No size</button>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, minmax(0, 1fr))', gap: 8 }}>
                {SIZE_OPTIONS.map((option) => <button key={option} className="work-size-option" type="button" data-selected={size === option} onClick={() => selectSize(option)} aria-pressed={size === option}>{option}</button>)}
              </div>
              <div style={{ marginTop: 6, paddingTop: 14, borderTop: '1px solid #e2e8f0' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr auto', gap: 8 }}>
                  <input id="custom-work-entry-size" ref={customSizeInputRef} className="work-size-custom-input" value={customSize} onChange={(event) => setCustomSize(event.target.value)} maxLength={100} placeholder="e.g. 40/120 or 40 × 120" onKeyDown={(event) => { if (event.key === 'Enter') { event.preventDefault(); addCustomSize(); } }} />
                  <button className="work-size-picker-control" type="button" onClick={addCustomSize} disabled={!customSize.trim()} style={{ padding: '0 16px', fontWeight: 900 }}>Use</button>
                </div>
              </div>
