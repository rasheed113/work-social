  ...
  return (
    <>
      <style>{`
        ...
        .worker-summary__card{position:relative;display:block;min-width:0;border:1px solid rgba(96,165,250,.17);border-radius:19px;background:linear-gradient(145deg,rgba(15,23,42,.42),rgba(15,23,42,.28));box-shadow:0 12px 23px rgba(2,6,23,.28),inset 0 1px 0 rgba(255,255,255,.08),inset 0 -1px 0 rgba(99,102,241,.10);text-align:left;cursor:pointer;font:inherit;overflow:hidden;isolation:isolate;transition:transform .18s ease,box-shadow .18s ease,border-color .18s ease}
        ...
        .worker-summary__card--week{grid-column:span 8;min-height:255px;padding:16px 17px;--accent:99,102,241;background:radial-gradient(circle at 50% 58%,rgba(59,130,246,.18),transparent 29%),radial-gradient(circle at 15% 20%,rgba(139,92,246,.14),transparent 35%),linear-gradient(145deg,rgba(23,37,84,.48),rgba(15,23,42,.28));border-color:rgba(99,102,241,.30)}
        .worker-summary__card--today{grid-column:span 4;padding:14px;--accent:6,182,212;background:radial-gradient(circle at 85% 12%,rgba(34,211,238,.20),transparent 42%),linear-gradient(145deg,rgba(8,47,73,.48),rgba(15,23,42,.28));border-color:rgba(34,211,238,.28)}
        .worker-summary__card--month{grid-column:span 4;padding:14px;--accent:168,85,247;background:radial-gradient(circle at 85% 12%,rgba(217,70,239,.20),transparent 42%),linear-gradient(145deg,rgba(59,7,100,.44),rgba(15,23,42,.28));border-color:rgba(217,70,239,.28)}
        ...
        .worker-summary__label{color:#67e8f9;text-shadow:0 0 10px rgba(34,211,238,.22);font-size:9px;font-weight:950;letter-spacing:.12em;text-transform:uppercase;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
        .worker-summary__value{display:block;margin-top:12px;color:#f0abfc;text-shadow:0 0 14px rgba(236,72,153,.24);font-size:clamp(22px,4vw,31px);line-height:1;font-weight:950;letter-spacing:-.065em}
        .worker-summary__period{display:block;margin-top:6px;color:#a5b4fc;font-size:9px;font-weight:750;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
        ...
        .worker-summary__mini-meta{display:flex;justify-content:space-between;gap:8px;margin-top:7px;color:#a5b4fc;font-size:8px;font-weight:850}
        .worker-summary__week-caption{position:absolute;left:15px;top:14px;color:#67e8f9;text-shadow:0 0 10px rgba(34,211,238,.2);font-size:8px;font-weight:950;letter-spacing:.13em;text-transform:uppercase}
        ...
        .worker-summary__ring-core{position:absolute;display:flex;flex-direction:column;align-items:center;justify-content:center;width:57%;aspect-ratio:1;border:1px solid rgba(129,140,248,.30);border-radius:31%;background:radial-gradient(circle at 35% 25%,rgba(99,102,241,.22),rgba(30,41,59,.72) 48%,rgba(15,23,42,.82));box-shadow:0 15px 31px rgba(37,99,235,.13),inset 0 1px 0 rgba(255,255,255,1),inset 0 -1px 8px rgba(99,102,241,.10)}
        .worker-summary__ring-label{color:#67e8f9;text-shadow:0 0 10px rgba(34,211,238,.24);font-size:8px;font-weight:950;letter-spacing:.14em;text-transform:uppercase}
        .worker-summary__ring-core strong{margin-top:6px;color:#f0abfc;text-shadow:0 0 14px rgba(236,72,153,.22),;font-size:clamp(28px,6vw,43px);line-height:.9;font-weight:950;letter-spacing:-.075em;text-shadow:0 1px 0 #fff,0 4px 10px rgba(37,99,235,.12)}
        .worker-summary__ring-core small{margin-top:7px;max-width:90px;color:#a5b4fc;font-size:8px;font-weight:800;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
        .worker-summary__legend{position:absolute;left:50%;bottom:2px;display:flex;gap:6px;transform:translateX(-50%);max-width:92%;padding:5px 8px;border:1px solid rgba(129,140,248,.22);border-radius:999px;background:rgba(15,23,42,.58);box-shadow:0 6px 14px rgba(37,99,235,.07);white-space:nowrap}
        .worker-summary__legend span{color:#c4b5fd;font-size:8px;font-weight:900}.worker-summary__legend b{color:#67e8f9;text-shadow:0 0 8px rgba(34,211,238,.2);font-weight:950}
        .worker-summary__metrics{display:grid;gap:6px;margin-top:13px}
        .worker-summary__metric{display:grid;grid-template-columns:7px minmax(0,1fr) auto;align-items:center;gap:7px;padding:7px 8px;border:1px solid rgba(148,163,184,.14);border-radius:10px;background:rgba(2,6,23,.30)}
        ...
        .worker-summary__metric-copy{display:flex;gap:5px;min-width:0;color:#c4b5fd;font-size:8px;font-weight:800}.worker-summary__metric-copy b{color:#67e8f9;font-size:8px;font-weight:950}.worker-summary__metric strong{color:#f0abfc;text-shadow:0 0 9px rgba(236,72,153,.2);font-size:12px;font-weight:950;white-space:nowrap}
        .worker-summary__lifetime{grid-column:1 / -1;display:flex;align-items:center;justify-content:space-between;gap:16px;padding:12px 15px;--accent:245,158,11;background:radial-gradient(circle at 8% 50%,rgba(245,158,11,.18),transparent 30%),radial-gradient(circle at 92% 50%,rgba(236,72,153,.13),transparent 32%),linear-gradient(110deg,rgba(69,39,8,.42),rgba(15,23,42,.34) 54%,rgba(59,7,100,.32));border-color:rgba(245,158,11,.28)}
        .worker-summary__lifetime-label{display:block;color:#a78bfa;text-shadow:0 0 9px rgba(167,139,250,.2);font-size:8px;font-weight:950;letter-spacing:.12em;text-transform:uppercase}.worker-summary__lifetime-value{display:block;margin-top:3px;color:#f0abfc;text-shadow:0 0 13px rgba(236,72,153,.22);font-size:clamp(20px,4vw,28px);line-height:1;font-weight:950;letter-spacing:-.055em}.worker-summary__lifetime-copy{flex:0 1 310px;color:#c4b5fd;font-size:9px;line-height:1.4;font-weight:700;text-align:right}
