    ...
    <main style={{ width: '100%', maxWidth: 900, margin: '0 auto', padding: '24px 14px 112px', boxSizing: 'border-box' }}>
      <header style={{ marginBottom: 20, display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 14 }}>
        <div>
          ...
          <h1 style={{ margin: '6px 0 0', fontSize: 'clamp(28px, 7vw, 42px)', letterSpacing: '-.04em' }}>{showHistory ? 'My Work History' : 'Home'}</h1>
          <p style={{ margin: '8px 0 0', color: '#64748b', lineHeight: 1.55 }}>{showHistory ? `${history.totalCount} persisted Work Entries. History loads progressively.` : 'Your persisted My Work record and real Work totals.'}</p>
        </div>
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', justifyContent: 'flex-end' }}>
          {showHistory && <button type="button" onClick={() => setShowHistory(false)} style={{ minHeight: 42, padding: '0 11px', borderRadius: 12, fontWeight: 800, cursor: 'pointer' }}>← Home</button>}
      ...
      {!dashboard.loading && !dashboard.workerProfileId ? (
        ...
        <>
          {!showHistory && <WorkerWorkSummaryCards totals={dashboard.totals} periodLabels={dashboard.periodLabels} onOpenHistory={() => setShowHistory(true)} />}

          <section style={{ marginTop: 16 }} aria-labelledby="worker-home-my-work">
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, marginBottom: 10 }}>
              <div>
                <h2 id="worker-home-my-work" style={{ margin: 0, fontSize: 19 }}>{showHistory ? 'All My Work Entries' : 'My Work Entries'}</h2>
                <p style={{ margin: '4px 0 0', color: '#64748b', fontSize: 12 }}>{showHistory ? 'Edit your own entries or hide them from your view without destroying the canonical record.' : 'Recent persisted records. Tap an entry to open its details/history.'}</p>
              </div>
              {!showHistory && <button type="button" onClick={() => setShowHistory(true)} style={{ minHeight: 38, padding: '0 11px', borderRadius: 11, fontWeight: 800, cursor: 'pointer' }}>History</button>}
            </div>
