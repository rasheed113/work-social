...
export function WorkerFinance() {
  ...
  return <main style={{ width: '100%', maxWidth: 900, margin: '0 auto', padding: '18px 14px 112px', boxSizing: 'border-box' }}>
    ...
    {notice && <div role="status" aria-live="polite" style={{ ...card, marginBottom: 12, padding: '10px 13px', color: '#166534', background: '#f0fdf4', borderColor: '#bbf7d0', fontSize: 13, fontWeight: 850 }}>{notice}<button type="button" onClick={() => setNotice(null)} aria-label="Dismiss message" style={{ float: 'right', border: 0, background: 'transparent', fontSize: 18, cursor: 'pointer' }}>×</button></div>}
    <section aria-label="Finance summary" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(180px,1fr))', gap: 10, marginBottom: 16 }}><SummaryCard label="Total Earnings" value={amountLabel(finance.summary.total_earnings)} /><SummaryCard label="Received" value={amountLabel(finance.summary.received)} /><SummaryCard label="Remaining" value={amountLabel(finance.summary.remaining)} /></section>
...
function SummaryCard({ label, value }: { label: string; value: string }) { return <section style={{ ...card, padding: 17 }}><div style={{ color: '#64748b', fontSize: 11, fontWeight: 850, letterSpacing: '.06em', textTransform: 'uppercase' }}>{label}</div><div style={{ marginTop: 6, fontSize: 'clamp(23px,6vw,32px)', fontWeight: 950, letterSpacing: '-.04em' }}>{value}</div></section>; }
