...
function FilterPopup({ filter, onSelect, onClose }: { filter: Filter; onSelect: (filter: Filter) => void; onClose: () => void }) {
  ...
  return <div className="finance-filter-overlay" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
    ...
    <style>{`
      ...
      .finance-filter-trigger{position:relative;isolation:isolate;display:inline-flex;align-items:center;justify-content:center;gap:8px;min-height:40px;max-width:100%;padding:0 13px;border:1px solid rgba(37,99,235,.3);border-radius:12px;background:linear-gradient(180deg,#ffffff 0%,#f9fdff 34%,#edf8fb 58%,#dceff4 100%);color:#123b50;font:inherit;font-size:12px;font-weight:900;letter-spacing:.01em;cursor:pointer;box-shadow:inset 0 2px 0 rgba(255,255,255,.98),inset 0 1px 5px rgba(255,255,255,.7),inset 0 -2px 0 rgba(14,116,144,.18),0 1px 0 rgba(255,255,255,.95),0 3px 0 rgba(71,85,105,.18),0 5px 8px rgba(15,23,42,.11),0 10px 18px rgba(14,116,144,.11),0 0 14px rgba(45,212,191,.08);transition:transform .13s ease,box-shadow .13s ease,background .13s ease;}
      .finance-filter-trigger::before{content:"";position:absolute;z-index:-1;left:3px;right:3px;top:2px;height:43%;border-radius:9px 9px 55% 55%;background:linear-gradient(180deg,rgba(255,255,255,.92),rgba(255,255,255,.16));box-shadow:inset 0 1px 0 rgba(255,255,255,.9);pointer-events:none;}
      .finance-filter-trigger::after{content:"";position:absolute;z-index:-1;left:5px;right:5px;bottom:-3px;height:4px;border-radius:0 0 9px 9px;background:linear-gradient(180deg,rgba(71,85,105,.18),rgba(14,116,144,.12));filter:blur(.15px);pointer-events:none;}
      .finance-filter-trigger:hover{transform:translateY(-2px);background:linear-gradient(180deg,#ffffff 0%,#fbfeff 34%,#eefafd 58%,#e2f4f8 100%);box-shadow:inset 0 2px 0 #fff,inset 0 1px 5px rgba(255,255,255,.78),inset 0 -2px 0 rgba(14,116,144,.18),0 2px 0 rgba(255,255,255,.95),0 5px 0 rgba(71,85,105,.18),0 8px 12px rgba(15,23,42,.12),0 13px 22px rgba(14,116,144,.13),0 0 17px rgba(45,212,191,.1);}
      .finance-filter-trigger:active{transform:translateY(2px) scale(.975);background:linear-gradient(180deg,#e5f1f4 0%,#f1f8fa 45%,#ffffff 100%);box-shadow:inset 0 3px 6px rgba(15,23,42,.16),inset 0 1px 0 rgba(255,255,255,.7),0 1px 0 rgba(255,255,255,.9),0 2px 3px rgba(15,23,42,.08);}
      .finance-filter-trigger:focus-visible,.finance-filter-option:focus-visible,.finance-filter-close:focus-visible{outline:3px solid rgba(37,99,235,.22);outline-offset:3px;}
