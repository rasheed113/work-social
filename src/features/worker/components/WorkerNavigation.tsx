  ...
  return (
    ...
    >
      {destinations.map((destination) => {
        ...
        return (
          <button key={destination.path} type="button" onClick={() => navigate(destination.path)} aria-current={active ? 'page' : undefined} style={{ position: 'relative', minHeight: 44, minWidth: 0, border: active ? '1px solid rgba(125,211,252,.5)' : '1px solid rgba(255,255,255,.07)', borderRadius: 12, color: '#fff', background: active ? 'rgba(59,130,246,.35)' : 'rgba(255,255,255,.035)', cursor: 'pointer', font: 'inherit', padding: '2px 4px' }}>
            <span aria-hidden="true" style={{ display: 'block', fontSize: 16, lineHeight: 1.05 }}>{destination.icon}</span>
            <span style={{ fontSize: 9, lineHeight: 1.1, fontWeight: 800 }}>{label}</span>
          </button>
        );
      })}
    </nav>
