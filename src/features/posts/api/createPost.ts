...
export async function createPost(profileId: string, content: string, attachments: PostAttachmentInput[] = [], location: PostLocationInput | null = null) {
  ...
  const { data: post, error: postError } = await supabase
    .from('posts')
    .insert({ profile_id: profileId, content: normalizedContent || null, latitude: location?.latitude ?? null, longitude: location?.longitude ?? null, location_name: location?.name ?? null })
    .select()
    .single();
