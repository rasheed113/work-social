...
const actionStyle: React.CSSProperties = { minHeight: 36, padding: '0 10px', border: '1px solid rgba(99,102,241,.14)', borderRadius: 12, background: 'linear-gradient(145deg, #fff, #eef2ff)', color: '#334155', fontWeight: 800, fontSize: 13, boxShadow: '0 4px 10px rgba(15,23,42,.07), inset 0 1px 1px rgba(255,255,255,.95)', cursor: 'pointer' };
...
export function CreatePostForm({ profileId, onCreated }: CreatePostFormProps) {
  ...
  return <section style={cardStyle}>
    ...
    <header style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8, marginBottom: 8 }}>
      <div><h2 style={{ margin: 0, fontSize: 17, fontWeight: 900, color: '#17202a', letterSpacing: '-.02em' }}>Create post</h2><small style={{ display: 'block', marginTop: 2, color: '#64748b', fontWeight: 600, fontSize: 12 }}>Share something with your community</small></div>
      <span aria-hidden="true" style={{ width: 34, height: 34, flexShrink: 0, display: 'grid', placeItems: 'center', borderRadius: 11, background: 'linear-gradient(145deg, #67e8f9, #6366f1 58%, #a855f7)', boxShadow: 'inset 0 2px 2px rgba(255,255,255,.48), inset 0 -4px 7px rgba(49,46,129,.24), 0 6px 13px rgba(99,102,241,.22)' }}>✦</span>
    </header>
    <textarea value={content} onChange={(event) => setContent(event.target.value)} placeholder="What's happening?" disabled={saving} style={{ width: '100%', minHeight: 64, height: 72, boxSizing: 'border-box', resize: 'vertical', padding: '10px 11px', border: '1px solid rgba(100,116,139,.18)', borderRadius: 14, outline: 'none', background: 'rgba(255,255,255,.9)', color: '#17202a', fontSize: 14, lineHeight: 1.4, boxShadow: 'inset 0 2px 7px rgba(15,23,42,.035), 0 3px 9px rgba(15,23,42,.03)' }} />
