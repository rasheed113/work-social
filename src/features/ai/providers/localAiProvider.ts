...
const DEFAULT_MAX_TOKENS = OFFLINE_DEFAULT_MAX_TOKENS; const DEFAULT_TEMPERATURE = 0.7; const DEFAULT_TOP_P = 0.9; const DEFAULT_CONTEXT_SIZE = OFFLINE_WLLAMA_CONTEXT_SIZE; const MAX_TOKENS = 2048; const MAX_CONTEXT_SIZE = 8192; const MAX_STOP_SEQUENCES = 8;
...
export class LocalAiProvider implements AiProvider {
  ...
  async getRoutingStatus(attachments: AiAttachment[] = []): Promise<AiProviderStatus> {
    ...
    // MODEL_READY is reached only after the existing verified-model handoff and Wllama load.
    // Do not re-run the 491 MB checksum/eligibility path for every warm message.
  ...
  async sendMessage(messages: AiMessage[], attachments: AiAttachment[] = [], options?: AiGenerationOptions): Promise<AiResponse> {
    ...
    try {
      ...
      // Deterministic Work Entry commands do not need a 256-token natural-language completion.
      // The existing bridge still performs confirmation creation/persistence and remains the only action path.
      ...
      offlineAiTrace('GENERATION_STARTED', { state: this.runtime.getStatus(), messageCount: boundedMessages.length, startedAtMs: nowMs() });
      ...
      offlineAiTrace('RESPONSE_RECEIVED', { provider: this.id, nonEmpty: true, completedAtMs: nowMs() });
