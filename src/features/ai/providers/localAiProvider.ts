...
export class LocalAiProvider implements AiProvider {
  ...
  async getRoutingStatus(attachments: AiAttachment[] = []): Promise<AiProviderStatus> {
    ...
    if (!eligibility.eligible) { const insufficientResources = eligibility.reasons.some((reason) => reason.code === 'INSUFFICIENT_RAM' || reason.code === 'INSUFFICIENT_STORAGE'); return { state: 'unavailable', provider: this.id, mode: this.mode, reason: eligibility.reasons.map((reason) => reason.message).join(' '), reasonCode: insufficientResources ? 'INSUFFICIENT_RESOURCES' : 'MODEL_INCOMPATIBLE' }; }
    try { await this.modelManager.getVerifiedModelReference(this.modelId); offlineAiTrace('MODEL_VERIFIED', { modelId: this.modelId, modelVersion: model.version, checksum: 'PASS' }); } catch (error) { const message = error instanceof Error ? error.message : 'The local model could not be verified.'; if (message.includes('LOCAL_MODEL_NOT_INSTALLED')) return { state: 'unavailable', provider: this.id, mode: this.mode, reason: message, reasonCode: 'MODEL_NOT_INSTALLED' }; return { state: 'unavailable', provider: this.id, mode: this.mode, reason: message, reasonCode: 'MODEL_INVALID' }; }
    const provenance = this.runtime === webLocalAi.runtime && this.modelManager === webLocalAi.modelManager ? webLocalAi.getProvenance() : null;
  ...
  async sendMessage(messages: AiMessage[], attachments: AiAttachment[] = [], options?: AiGenerationOptions): Promise<AiResponse> {
    ...
    try {
      ...
      offlineAiTrace('MODEL_READY', { state: this.runtime.getStatus(), modelId: this.modelId });
      ...
      offlineAiTrace('RESPONSE_RECEIVED', { provider: this.id, nonEmpty: true }); return { conversationId: messages[0]?.conversationId ?? '', message: response.text, pendingActions: [], provider: this.id, mode: this.mode };
...
function normalizeGenerationOptions(options?: AiGenerationOptions): Pick<InferenceRequest, 'maxTokens' | 'temperature' | 'topP' | 'contextSize' | 'stopSequences' | 'signal'> { const maxTokens = options?.maxOutputTokens ?? DEFAULT_MAX_TOKENS; const temperature = options?.temperature ?? DEFAULT_TEMPERATURE; const topP = options?.topP ?? DEFAULT_TOP_P; const contextSize = options?.contextSize ?? DEFAULT_CONTEXT_SIZE; const stopSequences = options?.stopSequences ?? []; if (!Number.isInteger(maxTokens) || maxTokens < 1 || maxTokens > MAX_TOKENS) throw new LocalInferenceRuntimeError('INFERENCE_FAILED', `maxTokens must be an integer between 1 and ${MAX_TOKENS}.`); if (!Number.isFinite(temperature) || temperature < 0 || temperature > 2) throw new LocalInferenceRuntimeError('INFERENCE_FAILED', 'temperature must be between 0 and 2.'); if (!Number.isFinite(topP) || topP <= 0 || topP > 1) throw new LocalInferenceRuntimeError('INFERENCE_FAILED', 'topP must be greater than 0 and at most 1.'); if (!Number.isInteger(contextSize) || contextSize < 256 || contextSize > MAX_CONTEXT_SIZE) throw new LocalInferenceRuntimeError('CONTEXT_TOO_LARGE', `contextSize must be between 256 and ${MAX_CONTEXT_SIZE}.`); if (stopSequences.length > MAX_STOP_SEQUENCES) throw new LocalInferenceRuntimeError('INFERENCE_FAILED', `At most ${MAX_STOP_SEQUENCES} stop sequences are supported.`); return { maxTokens, temperature, topP, contextSize, stopSequences, signal: options?.signal }; }
