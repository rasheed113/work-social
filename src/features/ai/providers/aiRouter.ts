import type { AiAttachment, AiGenerationOptions, AiMessage, AiProvider, AiProviderStatus, AiRoute, AiRoutingMode, AiRoutingReasonCode, AiResponse } from './contracts';
...
export class AiRouter {
  ...
  async route(mode: AiRoutingMode = 'auto', attachments: AiAttachment[] = []): Promise<AiRoute> {
    await validateImagesBeforeRouting(attachments);
    if (mode === 'online') {
      return { provider: 'gemini', mode: 'online', reasonCode: 'ONLINE_EXPLICIT', reason: 'Online mode explicitly selects Gemini.' };
    }
    const localStatus = await this.getLocalRoutingStatus(attachments);
    if (mode === 'offline') {
      if (localStatus.state !== 'ready') {
        throw new AiRoutingError(localStatus.reasonCode ?? 'LOCAL_RUNTIME_UNAVAILABLE', 'offline', 'local', localStatus.reason ?? 'Local AI is unavailable in offline mode.');
      }
    ...
    if (localStatus.state === 'ready') {
      return { provider: 'local', mode: 'offline', reasonCode: 'AUTO_LOCAL_SELECTED', reason: 'Local AI is executable and all model, integrity, device, resource, and attachment requirements are satisfied.' };
    }
