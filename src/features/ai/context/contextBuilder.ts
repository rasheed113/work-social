...
export function buildAiContext(conversation: AiConversation, request: AiContextRequest, options: AiContextOptions = DEFAULT_AI_CONTEXT_OPTIONS, availableMemories: AiMemory[] = []): AiContextResult {
  ...
  for (let index = conversation.messages.length - 1; index >= 0 && recentNewestFirst.length < maxRecentMessages; index -= 1) {
    ...
    if (usedCharacters + message.content.length <= options.maxCharacters) { recentNewestFirst.push(message); usedCharacters += message.content.length; }
    else truncated = true;
  }
  ...
  if (options.includeSummary && conversation.summary) {
    ...
    if (recentMessages.length + 2 <= options.maxMessages && usedCharacters + candidate.content.length <= options.maxCharacters) { summaryMessage = candidate; usedCharacters += candidate.content.length; }
    else truncated = true;
  }
  ...
  for (const memory of relevantMemories) {
    ...
    if (usedCharacters + candidate.content.length <= options.maxCharacters) { includedMemories.push(memory); usedCharacters += candidate.content.length; }
    else truncated = true;
  }
...
function selectRelevantMemories(memories: AiMemory[], request: string, explicitIds?: string[]): AiMemory[] {
  ...
  if (explicitIds) for (const id of explicitIds) { const memory = memories.find((item) => item.id === id); if (memory) unique.set(memory.id, memory); }
