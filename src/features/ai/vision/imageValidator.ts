...
function readDeclaredSize(metadata: Record<string, unknown> | undefined): number | null {
  if (!metadata) return null;
  const value = metadata.declaredSizeBytes ?? metadata.size;
...
function readJpegDimensions(bytes: Uint8Array): ImageDimensions {
  ...
  while (offset + 3 < bytes.length) {
    ...
    if (length < 2 || offset + length > bytes.length) return { width: null, height: null, verifiedFromBytes: false };
    const isSof = (marker >= 0xc0 && marker <= 0xc3) || (marker >= 0xc5 && marker <= 0xc7) || (marker >= 0xc9 && marker <= 0xcb) || (marker >= 0xcd && marker <= 0xcf);
    if (isSof && length >= 7) return dimensions((bytes[offset + 5] << 8) | bytes[offset + 6], (bytes[offset + 3] << 8) | bytes[offset + 4]);
    offset += length;
  }
  return { width: null, height: null, verifiedFromBytes: false };
}
