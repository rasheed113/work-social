...
export async function validateVisionImage(attachment: AiAttachment): Promise<ValidatedVisionImage> {
  ...
  if (attachment.data) {
    ...
    dimensions = await readImageDimensionsFromBlob(attachment.mimeType, attachment.data);
...
async function readImageDimensionsFromBlob(mimeType: string, blob: Blob): Promise<ImageDimensions> {
  if (mimeType === 'image/jpeg') return readJpegDimensions(new Uint8Array(await blob.arrayBuffer()));
  const headerLength = mimeType === 'image/png' ? PNG_HEADER_BYTES : WEBP_HEADER_BYTES;
  const header = new Uint8Array(await blob.slice(0, Math.min(headerLength, blob.size)).arrayBuffer());
  return readImageDimensions(mimeType, header);
}
...
function ascii(bytes: Uint8Array, offset: number, length: number): string { return String.fromCharCode(bytes[offset] ?? 0, bytes[offset + 1] ?? 0, bytes[offset + 2] ?? 0, bytes[offset + 3] ?? 0).slice(0, length); }
