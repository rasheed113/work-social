...
function parseQuantity(body: string): string | null { const afterMarker = parseNumeric(segment(body, QUANTITY_MARKER)); if (afterMarker) return afterMarker; const beforeMarker = body.match(/(?<![-\d.])(\d+(?:\.\d+)?)\s*(?:pieces?|pcs?|quantity|qty|count|numbers?|num|پیس(?:ز)?|ٹکڑے|تعداد|مقدار)/iu); return parseNumeric(beforeMarker?.[1] ?? null); }
