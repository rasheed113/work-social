...
const ACTION_PATTERN = /(?:\badd\b|\bcreate\b|\bnew\b|\bmake\b|\bbana(?:o|do)?\b|\bbna(?:o|do)?\b|\bdaal(?:o|do)?\b|\bentry\s+(?:bana|add|create)\b|\bentry\s+add\b|نئی|نیا|بنائیں|بناؤ|بناو|بنائیں)/iu;
...
function parseNumeric(value: string | null): string | null { if (!value) return null; const normalized = value.replace(/,/g, '').trim(); if (!/^\d+(?:\.\d+)?$/.test(normalized)) return null; const number = Number(normalized); if (!Number.isFinite(number) || number <= 0) return null; return normalized; }
...
export function detectLocalWorkAction(text: string, occurredAtIso: string): LocalWorkActionDetection {
  ...
  if (!item) {
    const prefixRemoved = stripActionPrefix(body);
  ...
  const size = sizeValue ? sizeValue.split(/\s*(?:,|and|&)\s*/iu).map(clean).filter(Boolean) : null;
