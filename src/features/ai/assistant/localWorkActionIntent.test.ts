...
function expectMissing(text: string, field: 'item_name' | 'quantity' | 'rate') {
  const result = detectLocalWorkAction(text, now);
  assert.equal(result.kind, 'missing');
  if (result.kind === 'missing') assert.equal(result.missing, field);
}
...
if (exact.kind === 'action') {
  assert.equal(exact.intent.toolName, 'create_work_entry');
  assert.deepEqual(exact.intent.arguments, {
    item_name: 'shirt', size: ['S'], quantity: '32', rate: '34', special_note: null, occurred_at_iso: now,
  });
  assert.equal(exact.total, 1088);
}

const variations = detectLocalWorkAction('nayi work entry banao item shirt size S rate 34 pieces 32', now);
assert.equal(variations.kind, 'action');
if (variations.kind === 'action') assert.equal(variations.intent.arguments.item_name, 'shirt');
