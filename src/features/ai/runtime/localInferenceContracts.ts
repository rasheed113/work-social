...
export type LocalInferenceErrorCode = 'OFFLINE_TEXT_AI_UNAVAILABLE' | 'MODEL_NOT_INSTALLED' | 'MODEL_INVALID' | 'MODEL_INCOMPATIBLE' | 'MODEL_DOWNLOAD_FAILED' | 'WLLAMA_WASM_FETCH_FAILED' | 'WLLAMA_COMPAT_WASM_FETCH_FAILED' | 'WLLAMA_WORKER_ASSET_FAILED' | 'MODEL_STORAGE_READ_FAILED' | 'MODEL_STORAGE_WRITE_FAILED' | 'RUNTIME_INITIALIZATION_FAILED' | 'RUNTIME_UNAVAILABLE' | 'MODEL_LOAD_FAILED' | 'INFERENCE_FAILED' | 'INFERENCE_CANCELLED' | 'CONTEXT_TOO_LARGE' | 'INSUFFICIENT_RESOURCES' | 'UNSUPPORTED_ATTACHMENT' | 'INVALID_STATE' | 'INVALID_MODEL_REFERENCE' | 'MODEL_NOT_READY' | 'VISION_NOT_SUPPORTED' | 'VISION_RUNTIME_UNAVAILABLE' | 'UNSUPPORTED_IMAGE_TYPE' | 'IMAGE_TOO_LARGE' | 'IMAGE_COUNT_EXCEEDED' | 'INVALID_IMAGE_METADATA';
