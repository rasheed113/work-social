...
export interface LocalInferenceProvenance { provider: 'local'; runtime: 'wllama'; model: string; source: LocalModelSource; verified: true; }
...
export const webLocalAi = {
  ...
  async prepare(onDownloadProgress?: (progress: ModelDownloadProgress) => void): Promise<void> {
    ...
    offlineAiTrace('MODEL_DISCOVERY', { database: 'work-social-ai-models', store: 'models' });
    preparePromise = preparePrimaryModel(modelManager, storage, downloader, runtime, reportProgress);
    try { await preparePromise; } finally { preparePromise = null; prepareProgressCallback = undefined; }
  },
  isPreparing(): boolean { return preparePromise !== null; },
...
export async function preparePrimaryModel(
  ...
  offlineAiTrace('MODEL_DISCOVERY', { modelId: model.id, modelVersion: model.version, modelState: model.status, stored: model.status === 'INSTALLED' });
  ...
  if (!installedAndVerified) {
    ...
    offlineAiTrace('MODEL_FOUND', { source: installedSource, modelId: model.id });
  }
  ...
  offlineAiTrace('MODEL_READY', { state: localRuntime.getStatus(), modelId: model.id, modelVersion: model.version, reused: false });
}
...
export function createReadyLocalDiagnostic(): LocalAiDiagnostic | undefined {
  ...
  return { stage: 'MODEL_LOAD', code: 'MODEL_READY', result: 'PASS', message: 'Verified GGUF is loaded by the wllama runtime and is ready for local inference.', filename: model.name, expectedBytes: model.sizeBytes, actualBytes: model.sizeBytes, sha256: model.sha256 ?? undefined, checksum: 'PASS', gguf: 'VALID', storage: 'SUCCESS', provider: provenance.provider, runtime: provenance.runtime, source: provenance.source, timestamp: new Date().toISOString(), modelId: model.id, modelVersion: model.version };
}
