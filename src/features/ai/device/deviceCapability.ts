...
export interface ModelEligibility { eligible: boolean; reasons: string[]; limitations: string[]; }
const GB = 1024 ** 3;
...
export function evaluateLocalModel(capability: DeviceCapability, requirements: LocalModelRequirements): ModelEligibility {
  const reasons: string[] = [];
  const limitations = [...capability.limitations];
  if (requirements.requiredPlatform === 'android' && capability.platform !== 'android') reasons.push('An Android/native runtime is required for local model execution.');
  if (!capability.supported) reasons.push(capability.reason ?? 'The current runtime does not support local AI execution.');
  if (capability.tier === 'UNKNOWN') reasons.push('Total RAM is unknown; local model eligibility cannot be determined conservatively.');
  if (requirements.minimumTotalRam !== undefined) {
    if (capability.totalRam === null) reasons.push('Total RAM is unknown.');
    else if (capability.totalRam < requirements.minimumTotalRam) reasons.push('Total RAM is below the model requirement.');
  }
  if (requirements.minimumAvailableRam !== undefined) {
    if (capability.availableRam === null) reasons.push('Available RAM is unknown.');
    else if (capability.availableRam < requirements.minimumAvailableRam) reasons.push('Available RAM is below the model requirement.');
  }
  if (requirements.minimumFreeStorage !== undefined) {
    if (capability.availableStorage === null) reasons.push('Available storage is unknown.');
...
export const PRIMARY_TEXT_MODEL_PLANNING_REQUIREMENTS: LocalModelRequirements = { minimumTotalRam: 4 * GB, minimumFreeStorage: 2.5 * GB, requiredPlatform: 'android' };
export const GIB = GB;
