...
type Worker = { worker_profile_id: string; display_name: string | null; username: string | null; avatar_url: string | null; completed_pieces: number; activity_events_7d: number; completed_7d: number; previous_completed_7d: number; last_activity_at: string | null; movement: Movement };
type Team = { team_id: number; team_number: number; team_name: string; worker_count: number; taken_pieces: number; completed_pieces: number; remaining_pieces: number; progress_pct: number | null; activity_events_7d: number; completed_7d: number; previous_completed_7d: number; movement: Movement; last_activity_at: string | null; lifetime_output_events: number; workers: Worker[] };
...
const initials = (name: string | null, username: string | null) => (name ?? username ?? 'W').trim().split(/\s+/).slice(0, 2).map(x => x[0]?.toUpperCase() ?? '').join('') || 'W';
...
const time = (value: string | null) => value ? new Intl.DateTimeFormat('en-PK', { dateStyle: 'medium', timeStyle: 'short' }).format(new Date(value)) : 'No recent activity';
...
export function ContractorTeamWorkerIntelligence() {
  ...
  return <section className="ctwi" aria-labelledby="ctwi-title">
    ...
    <header className="ctwi-head">
      <div><div className="ctwi-kicker">TEAM / WORKER INTELLIGENCE</div><h2 id="ctwi-title" className="ctwi-title">Where the business is moving.</h2><p className="ctwi-copy">Teams are ordered by completed output, then real taken volume, then recent activity. Worker movement uses the same real 7-day comparison.</p></div>
    ...
    <div className="ctwi-grid" aria-label="Team intelligence">
      {model.ordered.map((team, index) => {
        ...
        return <article className={`ctwi-team ${index === 0 ? 'leader' : ''}`} key={team.team_id}>
          ...
          <div className="ctwi-worker-head"><span>WORKER MOVEMENT</span><small>{team.workers.length ? `${team.workers.length} members` : 'NO ACTIVE WORKERS'}</small></div>
          {team.workers.length ? <div className="ctwi-workers">{team.workers.slice(0, 4).map(worker => <div className="ctwi-worker" key={worker.worker_profile_id}>
            <div className="ctwi-avatar">{worker.avatar_url ? <img src={worker.avatar_url} alt=""/> : <span>{initials(worker.display_name, worker.username)}</span>}</div>
            <div className="ctwi-worker-main"><strong>{worker.display_name ?? worker.username ?? 'Worker'}</strong><span>{worker.username ? `@${worker.username}` : 'Worker profile'}</span></div>
            <div className="ctwi-worker-stat"><strong>{pcs(worker.completed_pieces)}</strong><span>output</span></div>
            <div className={`ctwi-worker-move ${movementTone(worker.movement)}`} title={movementLabel(worker.movement)}>{worker.movement === 'increasing' ? '↗' : worker.movement === 'decreasing' ? '↘' : worker.movement === 'stable' ? '→' : '—'}</div>
          </div>)}</div> : <div className="ctwi-worker-empty">NO ACTIVE WORKERS</div>}
          {team.workers.length > 4 && <div className="ctwi-more">+{team.workers.length - 4} more workers in Team Workspace</div>}
