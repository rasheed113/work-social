...
type Team = {
  ...
  completed_7d: number;
  previous_completed_7d: number;
  movement: Movement;
  last_activity_at: string | null;
  lifetime_output_events: number;
};
...
export function ContractorTeamWorkerIntelligence() {
  ...
  const model = useMemo(() => {
    ...
    const maxCompleted = Math.max(1, ...ordered.map(team => num(team.completed_pieces)));
    const totalCompleted = ordered.reduce((sum, team) => sum + num(team.completed_pieces), 0);
    return { ordered, mostActive, maxCompleted, totalCompleted };
  }, [teams]);

  if (loading) return <section className="ctwi" aria-labelledby="ctwi-title"><style>{styles}</style><div className="ctwi-state"><div className="ctwi-orb"/><div><div className="ctwi-kicker">TEAM INTELLIGENCE</div><h2 id="ctwi-title" className="ctwi-title">Reading team movement…</h2><p className="ctwi-copy">Loading real team and output records.</p></div></div></section>;
  if (error) return <section className="ctwi" aria-labelledby="ctwi-title"><style>{styles}</style><div className="ctwi-state"><div className="ctwi-kicker">TEAM INTELLIGENCE</div><h2 id="ctwi-title" className="ctwi-title">Intelligence unavailable</h2><p className="ctwi-copy">{error}</p></div></section>;
  if (!model.ordered.length) return <section className="ctwi" aria-labelledby="ctwi-title"><style>{styles}</style><div className="ctwi-state"><div className="ctwi-kicker">TEAM INTELLIGENCE</div><h2 id="ctwi-title" className="ctwi-title">No teams yet</h2><p className="ctwi-copy">There are no contractor-owned teams available for business-wide intelligence.</p></div></section>;

  return <section className="ctwi" aria-labelledby="ctwi-title">
    <style>{styles}</style>
    <header className="ctwi-head">
      <div>
        <div className="ctwi-kicker">TEAM INTELLIGENCE</div>
        <h2 id="ctwi-title" className="ctwi-title">Where the business is moving.</h2>
        <p className="ctwi-copy">Teams are ordered by completed output, then real taken volume, then recent activity. Individual worker details stay inside each Team Dashboard.</p>
      </div>
    ...
    <div className="ctwi-grid" aria-label="Team intelligence">
      {model.ordered.map((team, index) => {
        ...
        return <article className={`ctwi-team ${index === 0 ? 'leader' : ''}`} key={team.team_id}>
          ...
          <div className="ctwi-output">
            <div className="ctwi-output-value">{pcs(team.completed_pieces)}</div>
            <div className="ctwi-output-label">completed pieces</div>
            <div className="ctwi-output-track"><span style={{ width: `${outputWidth}%` }}/></div>
          </div>
