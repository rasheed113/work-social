...
export function ProfilePanel({profileId,viewerId}:ProfilePanelProps){
 ...
 <div className="profile-actions">{isOwner?<><button className="profile-action-button" type="button" onClick={()=>navigate('/friends')}>Friends</button><button className="profile-action-button primary" type="button" onClick={openEditor}>Edit</button><button className="profile-action-button danger" type="button" onClick={()=>navigate('/blocked-users')}>🚫 Blocked Users</button></>:<><button className="profile-action-button primary" type="button" onClick={()=>void chatWithProfile()} disabled={busyAction}>💬 Chat with {profile.display_name}</button><button className="profile-action-button" type="button" onClick={()=>void toggleFollow()} disabled={busyAction}>{following?'Following':'Follow'}</button><button className="profile-action-button" type="button" onClick={()=>void sendFriendRequest()} disabled={busyAction||friend}>{friend?'Friends':friendPending?'Request sent':'Add as Friend'}</button><button className="profile-action-button danger" type="button" onClick={()=>void blockProfile()} disabled={busyAction}>🚫 Block</button></>}</div>{error&&<p className="profile-feedback" role="alert">{error}</p>}{saved&&<p className="profile-feedback" role="status">Profile saved.</p>}</section>
