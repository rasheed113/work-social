...
export function ExpenseManagerPage({ pathname, onNavigate }: ExpenseManagerPageProps) {
  ...
  return <main className="expense-manager-page">
    ...
    {section === 'overview' ? <ExpenseOverviewDashboard onNavigate={onNavigate} /> : section === 'transactions' ? <ExpenseTransactionsPage onNavigate={onNavigate} /> : section === 'accounts' ? <ExpenseAccountsPage onNavigate={onNavigate} /> : <section style={{ width:'min(1120px,100%)', margin:'0 auto', padding:'24px', boxSizing:'border-box', border:'1px solid rgba(148,163,184,.17)', borderRadius:'22px', background:'rgba(255,255,255,.9)', boxShadow:'0 14px 34px rgba(15,23,42,.07)' }} aria-label={`${copy.title} module boundary`}><h2 style={{margin:0,fontSize:'16px',fontWeight:900}}>Independent finance module</h2><p style={{margin:'7px 0 0',color:'#64748b',fontSize:'12px',lineHeight:1.6,fontWeight:600}}>This route remains intentionally scoped to the Expense Manager namespace. Phase 4 implements Accounts only; Categories, Budgets, and Reports remain separate Expense Manager phases.</p></section>}
  </main>;
}
