...
const sectionCopy: Record<ExpenseManagerSection, { title: string; description: string }> = {
  overview: {
    ...
    description: 'Understand your current position, period spending, accounts, budgets, and recent activity from persisted Expense Manager data.',
  },
  transactions: {
    title: 'Transactions',
    description: 'This route belongs to the Expense Manager module boundary. Transaction functionality is intentionally outside Phase 2.',
  },
  accounts: {
    title: 'Accounts',
    description: 'This route belongs to the Expense Manager module boundary. Account management is intentionally outside Phase 2.',
  },
  categories: {
    title: 'Categories',
    description: 'This route belongs to the Expense Manager module boundary. Category management is intentionally outside Phase 2.',
  },
  budgets: {
    title: 'Budgets',
    description: 'This route belongs to the Expense Manager module boundary. Budget management is intentionally outside Phase 2.',
  },
  reports: {
    title: 'Reports',
    description: 'This route belongs to the Expense Manager module boundary. Reporting and advanced analytics are intentionally outside Phase 2.',
  },
};
  ...
  return (
    <main className="expense-manager-page">
      <style>{`
        ...
        .expense-manager-page__hero{width:min(1120px,100%);margin:0 auto;padding:16px 2px 16px}
      ...
      {section === 'overview' ? (
        <ExpenseOverviewDashboard onNavigate={onNavigate} />
      ) : (
        <section className="expense-manager-page__card" aria-label={`${copy.title} module boundary`}>
          <h2 className="expense-manager-page__card-title">Independent finance module</h2>
          <p className="expense-manager-page__card-copy">This route remains intentionally scoped to the Expense Manager namespace. Phase 2 implements the Overview dashboard only; the selected section is not being silently expanded into a later module.</p>
          <div className="expense-manager-page__boundary">
            <span className="expense-manager-page__boundary-mark" aria-hidden="true">✓</span>
            <div>
              <strong>Phase 2 scope protected</strong>
              <span>Transactions, Accounts, Categories, Budgets, and Reports remain future Expense Manager phases.</span>
            </div>
          </div>
        </section>
      )}
