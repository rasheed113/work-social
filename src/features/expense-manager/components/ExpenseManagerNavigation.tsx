  ...
  return (
    <nav className="expense-manager-navigation" aria-label="Expense Manager navigation">
      <style>{`
        ...
        .expense-manager-navigation__item:focus-visible,.expense-manager-navigation__more-button:focus-visible{outline:2px solid rgba(37,99,235,.55);outline-offset:2px}
        ...
        .expense-manager-navigation__icon{font-size:16px;line-height:1;font-weight:900}
        ...
        .expense-manager-navigation__more-wrap{position:relative}
        .expense-manager-navigation__more-button{width:100%;min-height:48px;display:flex;align-items:center;justify-content:center;gap:5px;padding:0 5px;border:1px solid transparent;border-radius:13px;background:transparent;color:#475569;font:inherit;font-size:11px;font-weight:850;cursor:pointer}
        ...
        .expense-manager-navigation__more-panel{position:absolute;right:0;bottom:calc(100% + 8px);width:min(230px,calc(100vw - 28px));padding:7px;border:1px solid rgba(148,163,184,.2);border-radius:15px;background:rgba(255,255,255,.98);box-shadow:0 16px 36px rgba(15,23,42,.16);backdrop-filter:blur(18px);-webkit-backdrop-filter:blur(18px)}
        .expense-manager-navigation__more-item{width:100%;min-height:44px;display:flex;align-items:center;gap:9px;padding:0 10px;border:1px solid transparent;border-radius:11px;background:transparent;color:#334155;text-align:left;font:inherit;font-size:12px;font-weight:800;cursor:pointer}
        ...
        .expense-manager-navigation__quick-add{position:relative;min-height:48px;border:1px solid rgba(37,99,235,.22);border-radius:13px;background:linear-gradient(145deg,#2563eb,#4f46e5);color:#fff;font:inherit;font-size:16px;font-weight:950;box-shadow:0 8px 16px rgba(37,99,235,.18);cursor:not-allowed;opacity:.78}
        .expense-manager-navigation__quick-add:focus-visible{outline:2px solid rgba(37,99,235,.55);outline-offset:2px}
        @media (max-width:767px){
          .expense-manager-navigation{padding:0 8px 8px}
          .expense-manager-navigation__desktop{display:none}
          .expense-manager-navigation__mobile{display:block}
        }
        @media (max-width:380px){
          .expense-manager-navigation__mobile-grid{gap:3px;padding:5px}
          .expense-manager-navigation__item,.expense-manager-navigation__more-button,.expense-manager-navigation__quick-add{min-height:46px;font-size:10px}
          .expense-manager-navigation__icon{font-size:14px}
        }
      `}</style>
      <div className="expense-manager-navigation__shell">
          ...
          {sections.map((section) => {
            ...
            return (
              <button
                key={section.id}
                type="button"
                className="expense-manager-navigation__item"
                data-active={active}
                aria-current={active ? 'page' : undefined}
                onClick={() => navigate(section.path)}
              >
                <span className="expense-manager-navigation__icon" aria-hidden="true">{section.icon}</span>
        ...
        <div className="expense-manager-navigation__mobile">
          <div className="expense-manager-navigation__mobile-grid">
            ...
            <button type="button" className="expense-manager-navigation__quick-add" aria-label="Add transaction action reserved for a later phase" disabled>+</button>
            <div ref={moreRef} className="expense-manager-navigation__more-wrap" onKeyDown={handleMoreKeyDown}>
              <button
                type="button"
                className="expense-manager-navigation__more-button"
                data-active={moreOpen || mobileMore.some((section) => section.id === activeSection)}
                aria-expanded={moreOpen}
                aria-haspopup="menu"
                onClick={() => setMoreOpen((value) => !value)}
              >
                <span className="expense-manager-navigation__icon" aria-hidden="true">⋯</span>
                <span>More</span>
              </button>
              {moreOpen && (
                <div className="expense-manager-navigation__more-panel" role="menu" aria-label="More Expense Manager sections">
                  {mobileMore.map((section) => (
                    <button
                      key={section.id}
                      type="button"
                      role="menuitem"
                      className="expense-manager-navigation__more-item"
