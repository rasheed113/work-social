...
export function FriendsPage() {
  ...
  return <main><h1>Friends</h1><input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search people..." aria-label="Search people" />{error && <p role="alert">{error}</p>}{loading && <p>Loading people...</p>}{!loading && requests.length > 0 && <section><h2>Friend Requests</h2>{requests.map((request) => <article key={request.id}><strong>{profiles.find((p) => p.id === request.sender_id)?.display_name ?? profiles.find((p) => p.id === request.sender_id)?.username ?? 'User'}</strong><div><button type="button" onClick={() => void respond(request, 'accepted')}>Accept</button><button type="button" onClick={() => void respond(request, 'rejected')}>Reject</button></div></article>)}</section>}{!loading && <section><h2>{search.trim() ? 'Search results' : 'People'}</h2>{filtered.map((p) => { const isFriend = friendIds.has(p.id); const isPending = pendingIds.has(p.id); const isFollowing = followingIds.has(p.id); return <article key={p.id} style={{ display: 'flex', alignItems: 'center', gap: 10, marginTop: 12 }}>{p.avatar_url ? <img src={p.avatar_url} alt="" width={48} height={48} style={{ borderRadius: '50%', objectFit: 'cover' }} /> : <div aria-hidden="true" style={{ width: 48, height: 48, borderRadius: '50%', display: 'grid', placeItems: 'center', background: '#eee' }}>👤</div>}<button type="button" onClick={() => navigate(`/profile/${encodeURIComponent(p.id)}`)} style={{ flex: 1, border: 0, background: 'transparent', textAlign: 'left', padding: 0, cursor: 'pointer' }}><strong>{p.display_name ?? p.username ?? 'User'}</strong>{p.username && <div>@{p.username}</div>}{p.bio && <div>{p.bio}</div>}</button><button type="button" onClick={() => void toggleFollow(p.id)}>{isFollowing ? 'Following' : 'Follow'}</button><button type="button" onClick={() => isPending ? void cancelRequest(p.id) : void sendRequest(p.id)} disabled={isFriend}>{isFriend ? 'Friends' : isPending ? 'Cancel request' : 'Add friend'}</button></article>; })}{!filtered.length && <p>No people found.</p>}</section>}</main>;
}
