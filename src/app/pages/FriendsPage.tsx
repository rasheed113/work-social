...
const styles = {
  ...
  hero: {
    ...
    background: 'linear-gradient(135deg, rgba(5, 20, 40, .98) 0%, rgba(7, 37, 67, .97) 48%, rgba(8, 54, 76, .96) 100%)',
    boxShadow: '0 24px 70px rgba(0, 12, 32, .42), 0 0 38px rgba(38, 211, 255, .10), inset 0 1px 0 rgba(179, 245, 255, .16)',
    border: '1px solid rgba(73, 224, 255, .28)',
  } as React.CSSProperties,
  glow: { position: 'absolute', width: 230, height: 230, borderRadius: '50%', right: -80, top: -105, background: 'rgba(37, 211, 255, .20)', filter: 'blur(24px)', pointerEvents: 'none' } as React.CSSProperties,
  title: { margin: 0, fontSize: 32, lineHeight: 1.05, fontWeight: 900, letterSpacing: '.03em', color: '#effcff', textShadow: '0 0 8px rgba(109, 236, 255, .52), 0 0 26px rgba(35, 211, 255, .30)' } as React.CSSProperties,
  subtitle: { margin: '8px 0 0', color: 'rgba(211, 242, 250, .78)', fontSize: 14, lineHeight: 1.5, maxWidth: 620 } as React.CSSProperties,
  searchWrap: { display: 'flex', alignItems: 'center', gap: 10, marginTop: 18, padding: '11px 14px', borderRadius: 17, background: 'rgba(2, 16, 32, .72)', border: '1px solid rgba(72, 224, 255, .42)', boxShadow: '0 0 0 1px rgba(70, 216, 255, .05), 0 0 22px rgba(0, 204, 255, .10), inset 0 1px 0 rgba(190, 248, 255, .08)' } as React.CSSProperties,
  search: { flex: 1, minWidth: 0, border: 0, outline: 0, background: 'transparent', color: '#f5fdff', fontSize: 15, fontWeight: 650 } as React.CSSProperties,
  ...
  return (
    <main style={{ minWidth: 0, width: '100%', boxSizing: 'border-box' }}>
      <div style={styles.page}>
        <section style={{ ...styles.hero, backgroundImage: 'linear-gradient(rgba(100, 226, 255, .035) 1px, transparent 1px), linear-gradient(90deg, rgba(100, 226, 255, .035) 1px, transparent 1px), linear-gradient(135deg, rgba(5, 20, 40, .98) 0%, rgba(7, 37, 67, .97) 48%, rgba(8, 54, 76, .96) 100%)', backgroundSize: '30px 30px, 30px 30px, cover' }}><div style={styles.glow} /><div style={{ position: 'absolute', left: 22, right: 22, top: 14, height: 1, background: 'linear-gradient(90deg, transparent, rgba(93, 230, 255, .55), transparent)', opacity: .7 }} /><div style={{ position: 'relative', zIndex: 1 }}><div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}><div style={{ fontSize: 11, fontWeight: 900, letterSpacing: '.18em', textTransform: 'uppercase', color: '#79eaff' }}>SOCIAL NETWORK // FRIENDS</div><div style={{ display: 'inline-flex', alignItems: 'center', gap: 7, padding: '5px 9px', borderRadius: 999, background: 'rgba(0, 16, 30, .62)', border: '1px solid rgba(93, 231, 255, .24)', color: '#b9f5ff', fontSize: 10, fontWeight: 900, letterSpacing: '.12em', textTransform: 'uppercase' }}><span style={{ width: 6, height: 6, borderRadius: '50%', background: '#73f5c2', boxShadow: '0 0 10px rgba(115,245,194,.85)' }} /> LINK ONLINE</div></div><h1 style={styles.title}>Friends</h1><p style={styles.subtitle}>Connect with people, manage requests and build your circle.</p><div style={styles.searchWrap}><span aria-hidden="true" style={{ fontSize: 19, color: '#72eaff', textShadow: '0 0 10px rgba(72,224,255,.65)' }}>⌕</span><input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search people..." aria-label="Search people" style={styles.search} />{search && <button type="button" onClick={() => setSearch('')} aria-label="Clear search" style={{ border: '1px solid rgba(100, 231, 255, .22)', background: 'rgba(71, 215, 255, .09)', color: '#dffbff', borderRadius: 9, width: 30, height: 30, cursor: 'pointer', fontSize: 18, lineHeight: 1 }}>×</button>}</div></div></section>
