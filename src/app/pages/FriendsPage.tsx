...
export function FriendsPage() {
  ...
  const load = async () => {
    ...
    setFriendIds(new Set((friendships ?? []).map((f: any) => f.profile_a_id === userId ? f.profile_b_id : f.profile_a_id))); setLoading(false);
  };
  useEffect(() => { void load(); }, []);
  ...
  return <main><h1>Friends</h1><input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search people..." aria-label="Search people" />{error && <p role="alert">{error}</p>}{loading && <p>Loading people...</p>}{!loading && requests.length > 0 && <section><h2>Friend Requests</h2>{requests.map((request) => <article key={request.id}><strong>{profiles.find((p) => p.id === request.sender_id)?.display_name ?? profiles.find((p) => p.id === request.sender_id)?.username ?? 'User'}</strong><div><button type="button" onClick={() => void respond(request, 'accepted')}>Accept</button><button type="button" onClick={() => void respond(request, 'rejected')}>Reject</button></div></article>)}</section>}{!loading && <section><h2>{search.trim() ? 'Search results' : 'People'}</h2>{filtered.map((p) => { const isFriend = friendIds.has(p.id); const isPending = pendingIds.has(p.id); return <article key={p.id} style={{ display: 'flex', alignItems: 'center', gap: 10, marginTop: 12 }}>{p.avatar_url ? <img src={p.avatar_url} alt="" width={48} height={48} style={{ borderRadius: '50%', objectFit: 'cover' }} /> : <div aria-hidden="true" style={{ width: 48, height: 48, borderRadius: '50%', display: 'grid', placeItems: 'center', background: '#eee' }}>👤</div>}<div style={{ flex: 1 }}><strong>{p.display_name ?? p.username ?? 'User'}</strong>{p.bio && <div>{p.bio}</div>}</div><button type="button" onClick={() => isPending ? void cancelRequest(p.id) : void sendRequest(p.id)} disabled={isFriend}>{isFriend ? 'Friends' : isPending ? 'Cancel request' : 'Add friend'}</button></article>; })}{!filtered.length && <p>No people found.</p>}</section>}</main>;
}
