...
export function Router({ profileId }: RouterProps) {
  ...
  return <><header style={{ position: 'sticky', top: 0, zIndex: 900, display: 'flex', alignItems: 'center', gap: 8, marginBottom: 16, padding: '12px 14px', border: '1px solid rgba(0,0,0,.1)', borderRadius: 14, background: 'rgba(255,255,255,.96)', backdropFilter: 'blur(12px)' }}><strong style={{ marginRight: 'auto', fontSize: 18 }}>Work Social</strong><button type="button" onClick={() => navigate('/inbox')} aria-label={chatUnread ? `Work Social Chat, ${chatUnread} unread` : 'Work Social Chat'} style={{ fontWeight: chatUnread ? 700 : 400, borderColor: chatUnread ? '#e53935' : undefined }}>{chatUnread > 0 ? '🔴' : '💬'} Chat{chatUnread > 0 ? ` ${badge(chatUnread)}` : ''}</button></header>{pages[route]}<nav aria-label="Main navigation" style={{ position: 'fixed', left: 0, right: 0, bottom: 0, zIndex: 1000, display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 8, padding: '10px 12px calc(10px + env(safe-area-inset-bottom))', background: 'rgba(255,255,255,0.96)', borderTop: '1px solid rgba(0,0,0,0.12)', backdropFilter: 'blur(12px)' }}><button type="button" onClick={() => navigate('/')}>🏠 <span>Home</span></button><button type="button" onClick={() => navigate('/friends')}>👥 <span>Friends</span></button><button type="button" onClick={() => navigate('/notifications')} aria-label={notificationUnread ? `Notifications, ${notificationUnread} unread` : 'Notifications'}>🔔 <span>Notifications{notificationUnread > 0 ? ` (${badge(notificationUnread)})` : ''}</span></button><button type="button" onClick={() => navigate('/profile')}>👤 <span>Profile</span></button></nav></>;
}
