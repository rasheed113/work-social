...
export function Router({ profileId }: RouterProps) {
  ...
  const inboxPage:ReactNode=(<><InboxPage profileId={profileId}/><InboxGroupMaker profileId={profileId}/><InboxGroupMenu profileId={profileId}/><StableInboxCallControls profileId={profileId}/><CallSpeakerEnhancer /></>);
  ...
  const switchDestination=isWorkRoute?'/':'/work';
  return <><style>{`
    .ws-main-header{position:sticky;top:0;z-index:900;width:100%;max-width:100%;min-width:0;margin:0 0 18px;padding:9px 10px;box-sizing:border-box;display:flex;align-items:center;gap:10px;min-height:64px;overflow:hidden;background:linear-gradient(145deg,rgba(10,18,32,.96),rgba(25,35,55,.93) 52%,rgba(45,35,82,.92));border:1px solid rgba(255,255,255,.12);border-top-color:rgba(255,255,255,.2);border-radius:0 0 18px 18px;box-shadow:0 12px 30px rgba(15,23,42,.22),inset 0 1px 0 rgba(255,255,255,.09);backdrop-filter:blur(18px);-webkit-backdrop-filter:blur(18px)}
    ...
    .ws-main-header__brand,.ws-main-header__work,.ws-main-header__chat{position:relative;z-index:1}
    ...
    .ws-main-header__work{margin-left:auto;min-width:0;min-height:40px;flex:0 0 auto;display:inline-flex;align-items:center;justify-content:center;gap:6px;padding:0 13px;box-sizing:border-box;border:1px solid rgba(255,255,255,.2);border-radius:13px;color:#fff;background:linear-gradient(145deg,rgba(109,93,252,.9),rgba(37,99,235,.92) 54%,rgba(34,193,220,.88));box-shadow:inset 0 1px 1px rgba(255,255,255,.2),0 8px 18px rgba(79,70,229,.2);font-weight:800;cursor:pointer;white-space:nowrap;transition:transform .18s ease,box-shadow .18s ease,filter .18s ease}
    .ws-main-header__work:hover{filter:brightness(1.05);box-shadow:inset 0 1px 1px rgba(255,255,255,.22),0 10px 22px rgba(79,70,229,.26)}
    .ws-main-header__work:active{transform:translateY(1px);box-shadow:inset 0 2px 4px rgba(0,0,0,.18),0 5px 12px rgba(79,70,229,.18)}
    .ws-main-header__work:focus-visible,.ws-main-header__chat:focus-visible,.ws-main-header__brand:focus-visible{outline:2px solid rgba(125,211,252,.9);outline-offset:2px}
    .ws-main-header__work-icon{width:18px;height:18px;flex:0 0 18px;display:block}
    ...
    @media (max-width:420px){.ws-main-header{padding-left:8px;padding-right:8px;gap:7px}.ws-main-header__brand{gap:8px}.ws-main-header__logo{width:38px;height:38px;flex-basis:38px;font-size:17px}.ws-main-header__title{font-size:16px}.ws-main-header__tagline{font-size:7px;letter-spacing:.1em}.ws-main-header__work,.ws-main-header__chat{padding:0 10px;font-size:13px}.ws-main-header__badge{min-width:18px;height:18px}.ws-main-header__work-icon{width:17px;height:17px;flex-basis:17px}}
    @media (max-width:340px){.ws-main-header{gap:4px}.ws-main-header__brand{gap:5px;padding-left:0;padding-right:0}.ws-main-header__logo{width:34px;height:34px;flex-basis:34px;font-size:15px}.ws-main-header__title{font-size:13px}.ws-main-header__tagline{font-size:5px;letter-spacing:.065em}.ws-main-header__work,.ws-main-header__chat{padding:0 6px;font-size:11px;gap:3px}.ws-main-header__work-icon{width:16px;height:16px;flex-basis:16px}}
  `}</style><header className="ws-main-header"><button type="button" className="ws-main-header__brand" onClick={()=>navigate('/')} aria-label="Work Social home"><span className="ws-main-header__logo" aria-hidden="true">WS</span><span className="ws-main-header__brand-copy"><strong className="ws-main-header__title">Work Social</strong><span className="ws-main-header__tagline">Your Social Work Space</span></span></button><button type="button" className="ws-main-header__work" onClick={()=>navigate(switchDestination)} aria-label={isWorkRoute?'Return to Social Home':'Open Work House'}><svg className="ws-main-header__work-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.9" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d="M4 20V5.5A1.5 1.5 0 0 1 5.5 4H14"/><path d="M10 8h10v12H10z"/><path d="M14 4l3 3 3-3"/><path d="M14 4v6"/><path d="M4 16h6"/></svg><span>{switchLabel}</span></button><button type="button" className="ws-main-header__chat" onClick={()=>navigate('/inbox')}>💬 Chat {chatUnread>0&&<b className="ws-main-header__badge">{badge(chatUnread)}</b>}</button></header><div className={route==='inbox'?'work-social-page-content work-social-inbox-content':'work-social-page-content'} style={{width:'100%',boxSizing:'border-box',paddingBottom:route==='inbox'?0:92,overflowX:'clip'}}>{pages[route]}</div>{route==='work'?<WorkNavigation/>:<nav aria-label="Main navigation" style={{position:'fixed',left:10,right:10,bottom:'calc(10px + env(safe-area-inset-bottom))',zIndex:1000,display:'grid',gridTemplateColumns:'repeat(4,minmax(0,1fr))',gap:7,padding:8,background:'linear-gradient(145deg,rgba(15,23,42,.97),rgba(30,41,59,.96),rgba(49,46,129,.96))',border:'1px solid rgba(255,255,255,.16)',borderRadius:22,boxShadow:'0 16px 38px rgba(15,23,42,.34)'}}>{navItems.map(item=>{const active=route===item.route||(item.route==='profile'&&route==='publicProfile');return <button key={item.route} type="button" onClick={()=>navigate(item.path)} aria-current={active?'page':undefined} style={{position:'relative',minHeight:56,border:active?'1px solid rgba(125,211,252,.55)':'1px solid transparent',borderRadius:16,color:'#fff',background:active?'rgba(59,130,246,.35)':'transparent',cursor:'pointer'}}><span style={{display:'block',fontSize:20}}>{item.icon}</span><span style={{fontSize:10,fontWeight:800}}>{item.label}</span>{!!item.count&&<b style={{position:'absolute',top:4,right:8,minWidth:18,borderRadius:999,background:'#db2777',color:'#fff',fontSize:9}}>{badge(item.count)}</b>}</button>})}</nav>}</>;
}
