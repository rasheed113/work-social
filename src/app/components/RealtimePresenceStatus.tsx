...
export function RealtimePresenceStatus({ profileId }: { profileId: string }) {
  ...
  useEffect(() => {
    const updateHost = () => {
      setHost(document.querySelector('.inbox-call-actions'));
    };
    updateHost();
  ...
  useEffect(() => {
    ...
    const peerChannel = supabase.channel(`work-social-presence:${peer.id}`, {
  ...
  useEffect(() => {
    const ownChannel = supabase.channel(`work-social-presence:${profileId}`, {
      config: { private: true, presence: { key: profileId } },
    });
    ownChannel.subscribe(async (status) => {
      if (status === 'SUBSCRIBED') {
        await ownChannel.track({ userId: profileId, online_at: new Date().toISOString() });
      }
    });
